...
**
** $Id: si_voice.c 6479 2017-05-03 02:50:19Z nizajerk $
**
** Author(s):
** laj
**
...
#ifdef DEBUG
#undef TRACEPRINT
#undef TRACEPRINT_NOCHAN

#define TRACEPRINT(CHPTR,FMT,...) LOGPRINT("TRC:%s channel: %d "FMT,__FUNCTION__, (CHPTR)->channel,__VA_ARGS__)
#define TRACEPRINT2(CHPTR,FMT) LOGPRINT("TRC:%s channel: %d "FMT,__FUNCTION__, (CHPTR)->channel)
#define TRACEPRINT_NOCHAN(FMT,...) LOGPRINT("TRC:%s "FMT,__FUNCTION__, __VA_ARGS__)
#define TRACEPRINT_NOCHAN2(FMT,...) LOGPRINT("TRC:%s "FMT,__FUNCTION__)
#else
#define TRACEPRINT2(...)
#define TRACEPRINT_NOCHAN2(...)
#endif
...
uInt8 SiVoice_ReadReg(SiVoiceChanType_ptr hProslic,uInt8 addr)
{
  ...
  val = ReadReg(hProslic, hProslic->channel, addr);
  TRACEPRINT( hProslic, "addr = %u data = %u\n", (unsigned int)addr, val);
  return val;
}
