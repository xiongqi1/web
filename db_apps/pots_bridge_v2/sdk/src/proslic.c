...
**
** $Id: proslic.c 7060 2018-04-09 19:52:52Z nizajerk $
**
** Distributed by:
...
int ProSLIC_VerifyMasterStat(proslicChanType_ptr pProslic)
{
  ...
  if( regData != 0x1F )
  {
    DEBUG_PRINT(pProslic, "PCLK/FS not ready = 0x%02x\n", regData);
    return RC_SPI_FAIL;
  }
  else
  {
    DEBUG_PRINT(pProslic, "PCLK/FS ready = 0x%02x\n", regData);
    return RC_NONE;
  }
}
...
int ProSLIC_GetToneGenCfg(proslicChanType_ptr hProslic,int preset,ProSLIC_Tone_Cfg** tone)
{
#ifndef DISABLE_TONE_SETUP
  if(hProslic->channelType != PROSLIC)
  {
  ...
  if (hProslic->deviceId->chipType >= SI32171
      && hProslic->deviceId->chipType <= SI32179)
  {
    *tone=&(Si3217x_Tone_Presets[preset]);
    return 0;
  }
#endif
  ...
  if (hProslic->deviceId->chipType >= SI32180
      && hProslic->deviceId->chipType <= SI32189)
  {
    *tone=&(Si3218x_Tone_Presets[preset]);
    return 0;
  }
#endif
  ...
  if ( IS_SI3219X(hProslic->deviceId) )
  {
    *tone=&(Si3219x_Tone_Presets[preset]);
    return 0;
  }
#endif
  ...
  if (hProslic->deviceId->chipType >= SI32260
      && hProslic->deviceId->chipType <= SI32269)
  {
    *tone=&(Si3226x_Tone_Presets[preset]);
    return 0;
  }
#endif
#ifdef SI3228X
  if (hProslic->deviceId->chipType >= SI32280
      && hProslic->deviceId->chipType <= SI32289)
  {
    *tone=&(Si3228x_Tone_Presets[preset]);
    return 0;
  }
#endif
#else
...
int ProSLIC_ToneGenStop (proslicChanType_ptr pProslic)
{
  ...
  DEBUG_PRINT(pProslic,"%s%s [SLIC-REG] stop tone, [val 0x%02x] ==> [addr %d] (old=0x%02x) ", LOGPRINT_PREFIX, __FUNCTION__,new_data, PROSLIC_REG_OCON, data);

  return WriteReg(pProslic,pProslic->channel,PROSLIC_REG_OCON,new_data);
}
