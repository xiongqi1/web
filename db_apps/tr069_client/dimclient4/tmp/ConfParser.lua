...
function parseHandler(self, param)
	...
	while true do
		local arg, tokenType = self:_expect({ 'value', 'number', 'close-paren' }, 'Expected handler argument.')
	...
	param:setHandler(handler) -- performs handler validation/bind
--	_G.print('handler')
end
...
function parseDefinition(self, parent, isRoot)
	...
	elseif type == 'collection' and not isRoot then
