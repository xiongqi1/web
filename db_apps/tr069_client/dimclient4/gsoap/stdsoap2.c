...
{ struct soap *soap = (struct soap*)calloc(1,sizeof(struct soap));
  if (soap)
...
{ register void *p = calloc(1,size);
  if (soap)
  { register int h = soap_hash_ptr(p);
    register struct soap_mlist *mp = (struct soap_mlist*)calloc(1,sizeof(struct soap_mlist));
...
{ return soap_copy_context((struct soap*)calloc(1,sizeof(struct soap)), soap);
}
#endif
...
#endif
#ifndef UNDER_CE
  soap->recvfd = -1;    /* Dimark [20080724 0000055] */
  soap->sendfd = 1;
#else
  soap->recvfd = stdin;
  soap->sendfd = stdout;
...
#else
  soap_set_recv_logfile(soap, "DEBUG.log");
  soap_set_sent_logfile(soap, "DEBUG.log");
#endif
...
{
#ifdef SERVER_JAVA_MOC
  /* this server can't handle Array types */
...
{ struct soap_cookie *p;
  ...
  { if (p->modified || !p->env)
    { s = tmp;
      ...
      /* Dimark change. MOTIV ACS does not process original  */
      /* style Netscape cookie escape coding, so by default  */
      /* no longer create escape codes			     */
      /* [080923 0000074]				     */
#ifdef OLD_NETSCAPE_COOKIE
      if (p->name)
...
#else
      if (p->name && (strlen(p->name) < (size_t)(tmp+4064))) {
        sprintf(s, "%s", p->name );
        s += strlen(s);
      }
      if (p->value && *p->value && ((int)strlen(p->value) < tmp-s+4064)) {
        sprintf(s, "=%s", p->value );
        s += strlen(s);
      }
#endif 
...
/* 
   080820 Dimark
   Added new procedure soap_get_http_body() to support
   updated functionality in http_parse()
*/

#ifndef PALM_1
static int
soap_get_http_body(struct soap *soap, int err)
{
#ifndef WITH_LEAN
   register size_t l = 0, n = -1;
   register char *s;
   const char *t;
   /* get HTML body of HTTP error content */
   if (!(soap->mode & SOAP_ENC_ZLIB) && (soap->mode & SOAP_IO) !=  
SOAP_IO_CHUNK)
     n = soap->length;
#ifdef WITH_FAST
   soap->labidx = 0;                    /* use look-aside buffer */
...
#ifdef WITH_FAST
     register size_t i, k;
     if (soap_append_lab(soap, NULL, 0)) /* allocate more space in  
look-aside buffer if necessary */
       return 0;
     s = soap->labbuf + soap->labidx;   /* space to populate */
     k = soap->lablen - soap->labidx;   /* number of bytes available */
     soap->labidx = soap->lablen;       /* claim this space */
   ...
   *s = '\0';
   ...
   t = soap_save_block(soap, NULL, 0);
#endif
   return soap_set_receiver_error(soap, "HTTP Error", t, err);
#endif
   return SOAP_OK;
}
#endif
...
{ char header[SOAP_HDRLEN], *s;
  ...
  *soap->endpoint = '\0';
  ...
  /* 080820 Dimark old code removed */
  // if (k == 0 || (k >= 200 && k <= 299) || k == 400 || k == 500)
  //  return SOAP_OK;

  /* 080820 000000 
     New code added.  Provided by Robert van Engelen
     <engelen@genivia.com> to get around an observed
     problem of that a 401 message with a large body 
     content was leaving bytes in remaining SSL
     read buffer on the SSL side
  */
  if (k == 0 || k == 200)
    return SOAP_OK;
  /* 
     Status 201 (Created), 202 (Accepted), ... and 
     HTTP 400 and 500 errors require normal return, 
     so SOAP/XML body is parsed. Otherwise, try to 
     parse HTTP body.
  */
  if (soap->length > 0 || (soap->imode & SOAP_IO) == SOAP_IO_CHUNK)
    { if (((k > 200 && k <= 299) || k == 400 || k == 500))
	return SOAP_OK;
      if (soap_get_http_body(soap, k))
	return soap->error;
    }
...
//  if (soap_element_begin_in(soap, "SOAP-ENV:Envelope", 0))
  if (soap_element_begin_in(soap, ":Envelope", 0))
...
//  printf("soap_connect_command() endpoint=\"%s\" endpoint=%p\n",endpoint,endpoint);
...
{ register const char *s;
  ...
  /* Empty message should have ContentLenght 0 */
  if (count >= 0)
#else
  if (count > 0)
#endif
...
SOAP_FMAC2
#ifdef CLIENT_LISTENER
/* Redefine the soap_response() to soap_connect()
   that ServerCalls react like ClientCalls */
