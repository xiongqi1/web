...
static void http_da_session_cleanup(void);
...
static int http_da_post_header(struct soap *soap, const char *key, const char *val)
{
  ...
  if (key && !strcmp(key, "Authorization") && data->nonce ) // HH 
  {
    ...
    if (data->opaque)
    	...
    	if (qop)
    		sprintf(soap->tmpbuf, "Digest realm=\"%s\", username=\"%s\", nonce=\"%s\", uri=\"%s\", qop=\"%s\", nc=%s, cnonce=\"%s\", response=\"%s\"", soap->authrealm, soap->userid, data->nonce, soap->path, qop, ncount, cnonce, response);
    	else
    		sprintf(soap->tmpbuf, "Digest realm=\"%s\", username=\"%s\", nonce=\"%s\", uri=\"%s\", response=\"%s\"", soap->authrealm, soap->userid, data->nonce, soap->path, response);

    return data->fposthdr(soap, key, soap->tmpbuf);
  }

  /* server's HTTP Authorization response */
  if (key && !strcmp(key, "WWW-Authenticate") && !strncmp( val, "Digest" , 6))
  {
...
extern char 	buf401[SOAP_BUFLEN];
...
static void http_da_session_start(const char *realm, const char *nonce, const char *opaque)
{
  ...
  session = (struct http_da_session*)calloc(1,sizeof(struct http_da_session));
  if (session)
  {
