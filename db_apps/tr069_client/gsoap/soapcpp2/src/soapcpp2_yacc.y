...
type	: VOID		{ $$ = mkvoid(); }
	...
	| struct '{' s2 decls '}'
			{ if ((p = entry(classtable, $1->sym)) && p->info.typ->ref)
			  {	sprintf(errbuf, "struct '%s' already declared", $1->sym->name);
				semerror(errbuf);
			  }
			  else
			  {	p = reenter(classtable, $1->sym);
	...
	| UNION '{' s3 decls '}'
			{ sym = gensym("_Union");
			  ...
			  if ((p = entry(classtable, sym)))
			  {	if ((Table*) p->info.typ->ref)
				{	sprintf(errbuf, "union or struct '%s' already declared", sym->name);
					semerror(errbuf);
				}
				else
				{	p->info.typ->ref = sp->table;
	...
	| UNION ID	{ if ((p = entry(classtable, $2)))
			  {	if (p->info.typ->type == Tunion)
				...
				{	sprintf(errbuf, "'union %s' redeclaration", $2->name);
			  		semerror(errbuf);
			  		$$ = mkint();
				}
			  }
