...
{	Entry *p;
	...
	if (*dirpath)
	  fprintf(stdout, "Using project directory path: %s\n", dirpath);

	if (namespaceid)
	{ prefix = namespaceid;
	  fprintf(stdout, "Using code namespace: %s\n", namespaceid);
	}
	strcpy(base, prefix);
	if (cflag)
		s = ".c";
	...
	*/
	...
	if (Eptr->info.sto & Sreturn)
	 ...
	 { if (!strncmp(Eptr->sym->name, "__size", 6))
      ...
      { fprintf(fd,"\npublic:\n\tvirtual int soap_type() const { return %d; } /* = unique id SOAP_TYPE_%s */", typ->num, c_ident(typ));
	...
	/* friend causes problems linking static functions. Adding these friends could enable serializing protected/private members (which is not implemented)
  ...
  if (enumtable)
    ...
    { char *x;
      ...
      if (is_imported(p->info.typ))
        continue;
      fprintf(fheader, "\n\n#ifndef SOAP_TYPE_%s", c_ident(p->info.typ));
      fprintf(fheader, "\n#define SOAP_TYPE_%s (%d)",c_ident(p->info.typ),p->info.typ->num);	
...
{ Entry *p;
  ...
  { if (!strcmp(ns->name, "SOAP-ENV") || !strcmp(ns->name, "SOAP-ENC") || !strcmp(ns->name, "xsi") || !strcmp(ns->name, "xsd"))
	...
	{ if (name)
	    fprintf(stdout, "Using %s service name: %s\n", ns->name, name);
	  if (style)
	    fprintf(stdout, "Using %s service style: %s\n", ns->name, style);
	  else if (!eflag)
	    fprintf(stdout, "Using %s service style: document\n", ns->name);
	  if (encoding && *encoding)
	    fprintf(stdout, "Using %s service encoding: %s\n", ns->name, encoding);
	  else if (encoding && !*encoding)
	    fprintf(stdout, "Using %s service encoding: encoded\n", ns->name);
	  else if (!eflag)
	    fprintf(stdout, "Using %s service encoding: literal\n", ns->name);
	  if (URL)
	    fprintf(stdout, "Using %s service location: %s\n", ns->name, URL);
	  if (executable)
	    fprintf(stdout, "Using %s service executable: %s\n", ns->name, executable);
	  if (import)
	    fprintf(stdout, "Using %s schema import: %s\n", ns->name, import);
	  else if (URI)
	    fprintf(stdout, "Using %s schema namespace: %s\n", ns->name, URI);
          if (sp && sp->name)
	    sprintf(buf, "%s%s.wsdl", dirpath, ns_cname(name, NULL));
	  else
	    sprintf(buf, "%s%s.wsdl", dirpath, ns_cname(ns->name, NULL));
	  if (!wflag)
	  { fprintf(stdout, "Saving %s Web Service description\n", buf);
            fd = fopen(buf, "w");
	    if (!fd)
	  ...
	  { if (iflag)
	        ...
	        fprintf(stdout, "Saving %s client proxy class\n", buf);
                fd = fopen(buf, "w");
	        if (!fd)
	        ...
	        sprintf(buf, "%s%s%s.h", dirpath, prefix, name1);
	        fprintf(stdout, "Saving %s service class\n", buf);
                fd = fopen(buf, "w");
	        if (!fd)
	    ...
	    { if (!Sflag && sp && sp->name)
	      { sprintf(buf, "%s%s%s.h", dirpath, prefix, ns_cname(name, "Proxy"));
	        fprintf(stdout, "Saving %s client proxy\n", buf);
                fd = fopen(buf, "w");
	        if (!fd)
	      ...
	      { sprintf(buf, "%s%s%s.h", dirpath, prefix, ns_cname(name, "Object"));
	        fprintf(stdout, "Saving %s server object\n", buf);
                fd = fopen(buf, "w");
	        if (!fd)
	  ...
	  fprintf(stdout, "Saving %s namespace mapping table\n", buf);
          fd = fopen(buf, "w");
	  if (!fd)
    ...
    { sprintf(buf, "%s%s.xsd", dirpath, ns_convert(ns->name));
      fprintf(stdout, "Saving %s XML schema\n", buf);
      fd = fopen(buf, "w");
      if (!fd)
...
{ Entry *p, *q, *r;
  ...
  fprintf(fd, "\tvirtual ~%s() { if (soap) { soap_destroy(soap); soap_end(soap); soap_done(soap); free((void*)soap); } };\n", name1);
  fflush(fd);
...
{ Symbol *s;
  ...
  if (!response)
  { strcat(tmp, ".req.xml");
    fprintf(stdout, "Saving %s sample SOAP/XML request\n", tmp);
  }
  else
  { strcat(tmp, ".res.xml");
    fprintf(stdout, "Saving %s sample SOAP/XML response\n", tmp);
  }
  fd = fopen(tmp, "w");
  if (!fd)
  ...
  { if (typ->type == Tclass && !is_volatile(typ))
	...
	else
	{
      ...
      if (has_ptr(p->info.typ->ref))
        fprintf(fout,"\tsoap_embedded(soap, this->%s + i, SOAP_TYPE_%s);", p->sym->name, c_ident(p->info.typ->ref));
	...
	else
	{
      ...
      fprintf(fout,"\n\t\t{");
      if (has_ptr(p->info.typ->ref))
        fprintf(fout,"\tsoap_embedded(soap, a->%s + i, SOAP_TYPE_%s);", p->sym->name, c_ident(p->info.typ->ref));
      if (((Tnode*)p->info.typ->ref)->type == Tclass && !is_external(p->info.typ->ref) && !is_volatile(p->info.typ->ref) && !is_typedef(p->info.typ->ref))
...
{ int d;
  ...
  fflush(fout);
    ...
    { d = get_Darraydims(typ);
      ...
      { if (d)
          fprintf(fout,"soap_embed(soap, (void*)this, (struct soap_array*)&this->%s, %d, tag, SOAP_TYPE_%s);", p->sym->name, d, c_ident(typ));
        else
          fprintf(fout,"soap_embed(soap, (void*)this, (struct soap_array*)&this->%s, 1, tag, SOAP_TYPE_%s);", p->sym->name, c_ident(typ));
      }
      else if (d)
        fprintf(fout,"soap_embed(soap, (void*)a, (struct soap_array*)&a->%s, %d, tag, SOAP_TYPE_%s);", p->sym->name, d, c_ident(typ));
      else
        fprintf(fout,"soap_embed(soap, (void*)a, (struct soap_array*)&a->%s, 1, tag, SOAP_TYPE_%s);", p->sym->name, c_ident(typ));
    }
    else if (typ->type == Tclass && !is_external(typ) && !is_volatile(typ) && !is_typedef(typ))
      fprintf(fout,"soap_embed(soap, (void*)this, NULL, 0, tag, SOAP_TYPE_%s);", c_ident(typ));
    else
      fprintf(fout,"soap_embed(soap, (void*)a, NULL, 0, tag, SOAP_TYPE_%s);", c_ident(typ));
...
{ int i, j, d = 0;
  ...
  if (d)
    ...
    fprintf(fout,"\n\tid = soap_attachment(soap, tag, id, a, (struct soap_array*)&a->%s, a->id, a->type, a->options, 1, type, SOAP_TYPE_%s);", p->sym->name, c_ident(typ));
  else
  ...
  { case Tstruct:
	...
	{ for (p = t->list; p; p = p->next) 
	  ...
	  { 
	   ...
	   else if (!is_invisible(p->sym->name) && !is_primclass(typ) && p->info.typ->type != Tfun && !is_void(p->info.typ))
