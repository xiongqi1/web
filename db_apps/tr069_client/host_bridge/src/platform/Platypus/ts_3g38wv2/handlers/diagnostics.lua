...
----- [start] object UDPEchoConfig ------------------------------------------------------
	...
	['**.UDPEchoConfig.Interface'] = {
		...
		set = function(node, name, value)
			...
			if value:find('LANDevice') then
				luardb.set('UDPEchoConfig.InterfaceInternal', 'LAN')
			elseif value:find('WANDevice') then
				local enabled = luardb.get('link.profile.1.enable')
				local status  = luardb.get('link.profile.1.status')

				if enabled and status and enabled == '1' and status == 'up' then
					luardb.set('UDPEchoConfig.InterfaceInternal', '3GWAN')
				else
					luardb.set('UDPEchoConfig.InterfaceInternal', 'WAN')
				end
			end
