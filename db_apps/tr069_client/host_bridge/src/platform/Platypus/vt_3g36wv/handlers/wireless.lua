...
-- Videotron's requirement

			local autochannel = luanvramDB.get('AutoChannelSelect')

			if autochannel == nil then return '0' end

			autochannel = string.gsub(autochannel:gsub("^%s+", ""), "%s+$", "")

			if autochannel == '1' then return '0' end

			local channel = luanvramDB.get('Channel')
			if channel == nil then return '0' end

			local num_channel = tonumber(channel)
			local min, max = possiblechannel()

			if num_channel == nil then return '0' end
			if min == nil or max == nil then return '0' end

			if num_channel < tonumber(min) or num_channel > tonumber(max) then return '0' end
		...
		set = function(node, name, value)
			...
			if valueNum <= tonumber(max) and valueNum >= 0 then
				if valueNum == 0 then 
					luanvramDB.set('AutoChannelSelect', '1')
				else
					luanvramDB.set('AutoChannelSelect', '0')
				end
