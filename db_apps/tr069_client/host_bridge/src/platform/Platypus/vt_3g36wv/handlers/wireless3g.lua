...
--------------------------[start]:PeriodicPING--------------------------
function periodicPING_cb()
	os.execute("periodicping.sh")
end
--------------------------[ end ]:PeriodicPING--------------------------
...
-- Done
	['**.X_NETCOMM_COM.SNMP.Enable'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('snmp_enable')
		...
		set = function(node, name, value)
			...
			if value == "1" or value == "0" then
				luanvramDB.set('snmp_enable', value)
				luardb.set('service.snmp.enable', value)
			else
	...
	['**.X_NETCOMM_COM.SNMP.ReadonlyCommunityName'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('snmp_name_readonly')

			if retVal == nil or retVal:gsub("%s+", "") == "" then
				return ""
			else
				return retVal
			end
		...
		set = function(node, name, value)
			if value ~= nil then
				luanvramDB.set('snmp_name_readonly', value)
				luardb.set('service.snmp.name.readonly', value)
			end
			return 0
		end
	},
	['**.X_NETCOMM_COM.SNMP.ReadwriteCommunityName'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('snmp_name_readwrite')
		...
		set = function(node, name, value)
			if value ~= nil then
				luanvramDB.set('snmp_name_readwrite', value)
				luardb.set('service.snmp.name.readwrite', value)
			end
			return 0
		end
	},

	['**.X_NETCOMM_COM.SNMP.SNMPTraps.TrapDestinationIP'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('snmp_trap_dest')

			if retVal == nil or retVal:gsub("%s+", "") == "" then
				return ""
			elseif not isValidIP4(retVal) then
				return ""
			else
				return retVal
			end
		...
		set = function(node, name, value)
			...
			if value:gsub("%s+", "") == "" or isValidIP4(value) then
				luanvramDB.set('snmp_trap_dest', value)
				luardb.set('service.snmp.snmp_trap_dest', value)
				return 0
			else
				return cwmpError.InvalidArgument
			end
	...
	['**.X_NETCOMM_COM.SNMP.SNMPTraps.HeartbeatInterval'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('heartbeat_interval')
			...
			if retVal == "" then
				return "0"
			elseif string.find(retVal, "[^%d]") == nil then
				return retVal
			else
				return "0"
			end
		...
		set = function(node, name, value)
			...
			if string.find(value, "[^%d]") == nil then
				luanvramDB.set('heartbeat_interval', value)
				luardb.set('service.snmp.heartbeat_interval', value)
				return 0
			else
				return cwmpError.InvalidArgument
			end
	...
	['**.X_NETCOMM_COM.SNMP.SNMPTraps.PersistenceTime'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('trap_persist')
		...
		set = function(node, name, value)
			value = string.gsub(value:gsub("^%s+", ""), "%s+$", "")
			if string.find(value, "[^%d]") == nil then
				luanvramDB.set('trap_persist', value)
				luardb.set('service.snmp.trap_persist', value)
				return 0
			else
				return cwmpError.InvalidArgument
			end
	...
	['**.X_NETCOMM_COM.SNMP.SNMPTraps.RetransmissionTime'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('trap_resend')
		...
		set = function(node, name, value)
			value = string.gsub(value:gsub("^%s+", ""), "%s+$", "")
			if string.find(value, "[^%d]") == nil then
				luanvramDB.set('trap_resend', value)
				luardb.set('service.snmp.trap_resend', value)
				return 0
			else
				return cwmpError.InvalidArgument
			end
...
--------------------------[start]:PeriodicPING--------------------------
	['**.X_NETCOMM_COM.PeriodicPING.PriDestAddress'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_destaddr')
			if retVal == nil then return '' end
			return retVal
		end,
		set = function(node, name, value)
			if value == nil then return 0 end
			luanvramDB.set('periodic_destaddr', value)
			dimclient.callbacks.register('postSession', periodicPING_cb)
			return 0
		end
	},
	['**.X_NETCOMM_COM.PeriodicPING.SecDestAddress'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_destaddr2')
			if retVal == nil then return '' end
			return retVal
		end,
		set = function(node, name, value)
			if value == nil then return 0 end
			luanvramDB.set('periodic_destaddr2', value)
			dimclient.callbacks.register('postSession', periodicPING_cb)
			return 0
		end
	},
	['**.X_NETCOMM_COM.PeriodicPING.PeriodicPingTimer'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_pingtimer')
			retVal = string.gsub(retVal:gsub("^%s+", ""), "%s+$", "")
			if retVal == nil or retVal == '' then return "0" end
		...
		set = function(node, name, value)
			...
			if int_val == nil then return cwmpError.InvalidParameterValue end
			if int_val ~= 0 and int_val < 60 or int_val > 65535 then return cwmpError.InvalidParameterValue end

			luanvramDB.set('periodic_pingtimer', value)
			dimclient.callbacks.register('postSession', periodicPING_cb)
			return 0
		end
	},
	['**.X_NETCOMM_COM.PeriodicPING.PeriodicPingAccTimer'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_pingaccel')
			retVal = string.gsub(retVal:gsub("^%s+", ""), "%s+$", "")
			if retVal == nil or retVal == '' then return "0" end
			if tonumber(retVal) == nil then return "0" end
			return retVal
		...
		set = function(node, name, value)
			...
			local int_val = tonumber(value)
			if int_val == nil then return cwmpError.InvalidParameterValue end
			if int_val ~= 0 and int_val < 10 or int_val > 65535 then return cwmpError.InvalidParameterValue end

			luanvramDB.set('periodic_pingaccel', value)
			dimclient.callbacks.register('postSession', periodicPING_cb)
			return 0
		end
	},
	['**.X_NETCOMM_COM.PeriodicPING.FailCount'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_failcount')
			retVal = string.gsub(retVal:gsub("^%s+", ""), "%s+$", "")
			if retVal == nil or retVal == '' then return "0" end
			if tonumber(retVal) == nil then return "0" end
			return retVal
		...
		set = function(node, name, value)
			...
			local int_val = tonumber(value)
			if int_val == nil then return cwmpError.InvalidParameterValue end
			if int_val < 0 or int_val > 65535 then return cwmpError.InvalidParameterValue end

			luanvramDB.set('periodic_failcount', value)
			dimclient.callbacks.register('postSession', periodicPING_cb)
			return 0
		end
	},
	['**.X_NETCOMM_COM.PeriodicPING.PeriodicReboot'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_forcereset')
			retVal = string.gsub(retVal:gsub("^%s+", ""), "%s+$", "")
			if retVal == nil or retVal == '' then return "0" end
			if tonumber(retVal) == nil then return "0" end
			return retVal
		...
		set = function(node, name, value)
			...
			local int_val = tonumber(value)
			if int_val == nil then return cwmpError.InvalidParameterValue end
			if int_val ~= 0 and int_val < 5 or int_val > 65535 then return cwmpError.InvalidParameterValue end

			luanvramDB.set('periodic_forcereset', value)
			dimclient.callbacks.register('postSession', periodicPING_cb)
			return 0
		end
	},
	['**.X_NETCOMM_COM.PeriodicPING.RebootCount'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luanvramDB.get('periodic_rebootcount')
			retVal = string.gsub(retVal:gsub("^%s+", ""), "%s+$", "")
			if retVal == nil or retVal == '' then return "0" end
			if tonumber(retVal) == nil then return "0" end
			return retVal
		...
		set = function(node, name, value)
			if value == nil then return cwmpError.InvalidParameterValue end
			value = string.gsub(value:gsub("^%s+", ""), "%s+$", "")
			if tonumber(value) == nil then return cwmpError.InvalidParameterValue end
			luanvramDB.set('periodic_rebootcount', value)
		end
	},
--------------------------[ end ]:PeriodicPING--------------------------
