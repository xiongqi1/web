...
local function validateParameterValue(name, value)
	...
	if not validator then
		error('No validator rule for parameter name "' .. paramName .. '".')
	end
	...
	elseif type(validator) == 'function' then
		...
		error('Not sure how to handle this kind of validator? ' .. type(validator))
	end
	return 0
end
	...
	['**.AVC.*.*'] = {
		init = function(node, name, value)
			...
			if node.value == nil then
				dimclient.log('info', 'AVC: defaulting ' .. key)
				node.value = node.default
			end
