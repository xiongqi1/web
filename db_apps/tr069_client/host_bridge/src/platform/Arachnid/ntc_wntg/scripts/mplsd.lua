...
function action.SHELL(v)
  execlog('sh ' ..v.. '|'.. conf.wntd.switchConfig)
end
...
local function addMpls(ind, trunk_vid, avc)
  ...
  local rdMode = unid:getMapTag()
  if avc.mtMode ~= rdMode or mode ~= rdMode then
      complain(false, 'addMpls mode or mtMode unid port in wrong mode '..trunk_vid, avc)
      return nil
  end
  ...
  local mpcode = { ['MPLS'] = 'start '..conf.wntd.switchIface..' '..trunk_vid..' '..ourLocalGreAddress..' '..avc.peer_address..' '..avc.mpls_tag..' '..flags..' '..service}
  addVlanQ(mpcode)
  local qdcmd = { ['QDISC'] = 'start '..qdisc}
  addVlanQ(qdcmd)
...
local function avcInit()
  ...
  avc_tab = rdbtable(conf.wntd.avcPrefix)
  if conf.wntd.debug > 2 then
...
local function checkAvcAddition(ind)
  ...
  avc.ind = ind
  if not avc['enable'] or (tonumber(avc['enable']) < 1) then
    avcstatusgood(avc, 'Disabled')
    return
  end
