...
# 2011-01-05 IwoM@netcomm.com.au
#   Removed source field checking
set -e
...
# Iwo: skipping .svn directories for control & data
echo ".svn" >> $tmp_dir/tarX
echo "Skip list"
cat $tmp_dir/tarX
echo "data archive:"
( cd $pkg_dir && tar $ogargs -cvzf $tmp_dir/data.tar.gz -X $tmp_dir/tarX .  )
echo "control archive:"
( cd $pkg_dir/$CONTROL && tar $ogargs -cvzf $tmp_dir/control.tar.gz -X $tmp_dir/tarX .  )
rm $tmp_dir/tarX
