...
static int qmi_proxy_init(void) {
    ...
    PQCQMIMSG pQMI = &_QMI;
    PQCQMIMSG pRsp;
    uint8_t TransactionId = 0xC1; //为什么这里是C1？
    uint8_t WDAClientId = 0;
    unsigned rx_urb_size = 4096; //must same as rx_urb_size defined in GobiNet&qmi_wwan driver
    unsigned ep_type, iface_id;
    unsigned qmap_version = 0;
    ...
    for (i = 0; i < 10; i++) {
        ...
        pQMI->CTLMsg.SyncReq.QMICTLType = QMICTL_SYNC_REQ;	//Synchronizes the service provider and service consumer
    ...
    qmap_version = 1;    //debug
    rx_urb_size = 2048;  //debug
    if (!qmap_version) {
        rx_urb_size = 16 * 1024; //must same as rx_urb_size defined in GobiNet&qmi_wwan driver
        ep_type = 0x02;
        iface_id = 0x04;
    } else {
        rx_urb_size = 4 * 1024;  //must same as rx_urb_size defined in GobiNet&qmi_wwan driver
        ep_type = 0x05;        //DATA_EP_TYPE_BAM_DMUX
        iface_id = 0x08;
    }
...
static void *qmi_proxy_loop(void *param)
{
    ...
    while (cdc_wdm_fd > 0) {
        ...
        do {
            ...
            ret = poll(pollfds, nevents, 100);
         } while (ret < 0 && errno == EINTR);

        if (ret < 0) {
            syslog(LOG_ERR, "%s poll=%d, errno: %d (%s)\n", __func__, ret, errno, strerror(errno));
        ...
        for (ne = 0; ne < nevents; ne++) {
            ...
            if (fd == qmi_proxy_server_fd) {
                ...
                uint8_t *p = (uint8_t*)pQMI;
                uint16_t len = pQMI->QMIHdr.Length + 1;
                while (nreads && nreads >= len) {
                    if (verbose_debug) {
                        ssize_t i;
                        printf("r %d %zd: ", fd, len);
                        for (i = 0; i < (len < 16 ? len : 16); i++)
                            printf("%02x ", ((uint8_t *)p)[i]);
                        printf("\n");
                    }
                    send_qmi((PQCQMIMSG)p, len, fd);

                    nreads -= len;  // finish one QMI message
                    p += len;       // get next message
                    len = ((PQCQMIMSG)p)->QMIHdr.Length + 1;
                }

                if (nreads != 0) {
                    syslog(LOG_WARNING, "WARN: partial message received, %d bytes dropped\n", nreads);
                }
            }
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    if (cdc_wdm_fd == -1) {
        syslog(LOG_ERR, "%s Failed to open %s, errno: %d (%s)\n", __func__, cdc_wdm, errno, strerror(errno));
        return -1;
    }
