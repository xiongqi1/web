/*
* @Author: Xushen Ma
* @Date:   2018-02-05 11:14:18
* @Last Modified by:   xushen
* @Last Modified time: 2018-02-12 11:42:13
*/
...
void RdbSimulator::SetupRdbActionTracking(const char *path)
{
    ...
    ON_CALL(*rdb, rdb_getinfo(NotNull(), StrEq(path), _, _, _))
        ...
        /* Actual check */
        return (rdbKey && rdbKey->Exist()) ? 0 : -ENOENT;
    }));
...
void RdbHandler::ExpectRdbAddOrUpdate_Add(const char *path, const char *value)
{
    Sequence seq;
    ExpectRdbGetAlloc(seq, -ENOENT, path, NULL);
    ExpectRdbCreate(seq, 0, path, value);
    EXPECT_CALL(*rdb, rdb_set(NotNull(), StrEq(path), StrEq(value), Ge(0))).Times(0);
}

void RdbHandler::ExpectRdbAddOrUpdate_Skip(const char *path, const char *value)
{
    ExpectRdbGetAlloc(0, path, value);

    // No call
    EXPECT_CALL(*rdb, rdb_create(NotNull(), StrEq(path), StrEq(value), Ge(0),_,_)).Times(0);
    EXPECT_CALL(*rdb, rdb_set(NotNull(), StrEq(path), StrEq(value), Ge(0))).Times(0);
}

void RdbHandler::ExpectRdbAddOrUpdate_Overwrite(const char *path, const char *old_value, const char *new_value)
{
    Sequence seq;
    ExpectRdbGetAlloc(seq, 0, path, old_value);
    ExpectRdbSet(seq, 0, path, new_value);

    // No call
    EXPECT_CALL(*rdb, rdb_create(NotNull(), StrEq(path), StrEq(new_value), Ge(0),_,_)).Times(0);
}

void RdbHandler::ExpectRdbAddOrUpdate_FailGet(const char *path)
{
    ExpectRdbGetAlloc(-ENOMEM, path, NULL);
...
void RdbHandler::ExpectRdbAddOrUpdate_FailCreate(const char *path, const char *value)
{
    Sequence seq;
    ExpectRdbGetAlloc(seq, -ENOENT, path, NULL);
    ExpectRdbCreate(seq, -EOVERFLOW, path, value);

    // No call
    EXPECT_CALL(*rdb, rdb_set(NotNull(), StrEq(path), StrEq(value), Ge(0))).Times(0);
}

void RdbHandler::ExpectRdbAddOrUpdate_FailSet(const char *path, const char *old_value, const char *new_value)
{
    Sequence seq;
    ExpectRdbGetAlloc(seq, 0, path, old_value);
    ExpectRdbSet(seq, -EBUSY, path, new_value);
