...
int strbt_del(struct strbt_t* srl,const char* str)
{
	...
	// search rbt
	if( !(rbt=_find_rbt(srl,hash_key)) ) {
		NTCLOG_ERR("rbt not found - key=%d",hash_key);
		goto err;
	}
	
	// search dl
	if( !(dl=_find_dl_in_rbt(srl,rbt,str)) ) {
		NTCLOG_ERR("dl not found - key=%d,str=%s",hash_key,str);
		goto err;
	}
...
int strbt_add(struct strbt_t* srl,const char* str,void* ref)
{
	...
	if( !(rbt=_find_rbt(srl,hash_key)) ) {
		if( !(rbt=_create_rbt()) ) {
			NTCLOG_ERR("rbt memory allocation failure - %s",strerror(errno));
			goto err;
		}
		...
		if( (dl=_find_dl_in_rbt(srl,rbt,str))!=0 ) {
			NTCLOG_ERR("str already exists - key=%ld,str=%s",hash_key,str);
			goto err;
		}
	}
	...
	if( !(dl=_create_dl(srl->ref_len,str)) ) {
		NTCLOG_ERR("dl memory allocation failure - %s",strerror(errno));
		goto err;
	}
