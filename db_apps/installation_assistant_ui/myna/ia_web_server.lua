...
local g_ia_dir = "/usr/share/installation_assistant/myna/"
...
local RegisterMethod= class("RegisterMethod", turbo.web.RequestHandler)
local UpdateAddress = class("UpdateAddress", turbo.web.RequestHandler)
...
function UserConfDataEntry:post(url)
    ...
    local sas_select = self:get_argument("sas_select", "", true)
    if sas_select == "0" then
      luardb.set("service.sas_client.enabled_by_ia", "0")
      -- redirect to RF page
    ...
    local reg_method_selected = self:get_argument("reg_method", "single", true)

    local reg_method_config = luardb.get("sas.config.regMethod") or ""
    if "" ~= reg_method_config and reg_method_config ~= reg_method_selected then
        reg_method_selected = reg_method_config
    end
    ...
    luardb.set("service.sas_client.enabled_by_ia", "1")
    ...
    luardb.set("sas.regMethod", reg_method_selected)
...
function UserConfDataEntry:get(url)
    ...
    data.owa_prod_model = {luardb.get(config.get_rdb_prefix.."system.product.model")      or ""}
    data.reg_method_selected = {luardb.get("sas.regMethod") or luardb.get("sas.config.regMethod") or "single"}

    local sas_enable = luardb.get(config.get_rdb_prefix.."service.sas_client.enabled_by_ia") or "0"
    data.sas_select = {sas_enable}
...
-- handle queries for register mode support and selection
function RegisterMethod:get()
    local response = {
        multi_step_reg_support = luardb.get("sas.config.canMultiStepReg") or "0",
        reg_method_config = luardb.get("sas.config.regMethod") or "",
        reg_method_selected = luardb.get("sas.regMethod") or ""
    }
...
-- handle queries to change email/sms address
function UpdateAddress:post()
    local key = self:get_argument("key", "", true)
    local address = self:get_argument("address", "", true)

    if key == "email" then
        luardb.set("sas.config.cpiEmail", address)
    end
    if key == "sms" then
        luardb.set("sas.config.cpiSms", address)
    end
end
...
function SasRegister:post(url)
    local single_step = "multi" ~= luardb.get("sas.regMethod")

    if single_step and not m_cbrs.cpi_key_available() then
        self:set_status(400)
        return
    end
    ...
    if (string.len(params.cpi_id) > 256 and single_step) then
      llog.log("LOG_ERR",string.format("CPI ID length %d is too long", string.len(params.cpi_id)))
      self:set_status(400)
      return
    end
    if (string.len(params.cpi_name) > 256 and single_step) then
    ...
    if not m_cbrs.invoke_registration(reg_params, not single_step) then
        self:set_status(400)
        return
    end
    ...
    if not single_step then
        ...
        measCapStr = {"", "RECEIVED_POWER_WITHOUT_GRANT", "RECEIVED_POWER_WITH_GRANT"}
        measCapIndex = (tonumber(luardb.get('sas.config.measCapability')) or 0) + 1

        reg_params.fcc_id = luardb.get("sas.config.fccid") or ""
        reg_params.cat = luardb.get("") or "B"
        reg_params.call_sign = luardb.get("sas.config.callSign") or "CB987"
        reg_params.user_id =  luardb.get("sas.config.userId") or ""
        reg_params.radio_tech = luardb.get("") or "E_UTRA"
        reg_params.serial = luardb.get("sas.config.cbsdSerialNumber") or ""
        reg_params.meas_caps = measCapStr[measCapIndex]
        reg_params.depolyment = luardb.get("sas.config.indoorDeployment") or ""
        reg_params.gain = luardb.get("sas.antenna.gain") or ""
        reg_params.beam_width = luardb.get("sas.antenna.beam_width") or ""

        reg_params.vendor = luardb.get("owa.system.product.vendor") or "Casa Systems Inc"
        reg_params.model = luardb.get("owa.system.product.model") or "undefined"
        reg_params.sw_ver = luardb.get("owa.sw.version") or "undefined"
        reg_params.hw_ver = luardb.get("wwan.0.hardware_version") or "undefined"
        reg_params.fw_ver = luardb.get("wwan.0.firmware_version") or "undefined"

        reg_params.imei = luardb.get("wwan.0.imei") or ""
        reg_params.imsi = luardb.get("wwan.0.imsi.msin") or ""
        reg_params.email_addr = luardb.get("sas.config.cpiEmail") or ""
        reg_params.sms_addr = luardb.get("sas.config.cpiSms") or ""

        self:write(reg_params)
    end
end
...
function SasDeregister:get(url)
    ...
    local command = "deregister"
    local force = self:get_argument("force", "false", true)
    local state = luardb.get("sas.registration.state")
    local executing = luardb.get("sas.registration.cmd") or ""

    if "true" == force then
        if state == "Unregistered" or executing ~= "" then
            return
        end
        command = "force_deregister"
    end

    -- Invoke de-registration
    luardb.set("sas.registration.cmd", command)
end
