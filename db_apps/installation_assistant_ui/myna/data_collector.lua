...
--[[
Given pci, earfcn find all PLMN info in the form of {mcc,mnc}
Uses rrc_info data structures.
]]--
function get_cell_plmn_list(pci, earfcn)
    local plmn_list = {}
    for i=1,#g_rrc_info,1 do
        if g_rrc_info[i].pci == pci and g_rrc_info[i].earfcn == earfcn then
            table.insert(plmn_list, {mcc=g_rrc_info[i].mcc, mnc=g_rrc_info[i].mnc})
            if g_rrc_info[i].has_dual_plmn then
                table.insert(plmn_list, g_rrc_info[i].plmn2)
            end
        end
    end
    return plmn_list
...
local function get_rsrp_pass_level(cell_sector_id)
    ...
    local ret = g_user_selected_rsrp_pass[1]
    ...
    if last_manual_cell_scan_seq ~= rf_scan_cell_seq_rdb then
        ...
        for i=1, rf_scan_cell_qty_rdb do
            ...
            if cell_measurement and #cell_measurement == 5 then
                ...
                g_current_rf_readings[i].cell_sector_id, g_current_rf_readings[i].cell_id =
                    get_cell_sector_id_by_pci_earfcn(g_current_rf_readings[i].pci, g_current_rf_readings[i].earfcn)
                ...
                if #l == 5 then
                    ...
                    if is_new then
                        ...
                        if index >= 1 and index <= #g_rrc_info then
                            ...
                            g_rrc_info[index].plmn2 = {mcc=l[1], mnc=l[2]} -- store the second PLMN info too
                        end
                    end
                end
            end
...
local function get_registration_status()
    ...
    server_data.modem_tx_status = luardb.get("wwan.0.txctrl.tx_state") or "1"
    server_data.modem_next_tx_time = luardb.get("wwan.0.txctrl.next_tx.time") or ""
    server_data.modem_tx_update_time = luardb.get("wwan.0.txctrl.tx_state_chg.time") or ""
    server_data.nit_tx_update_time = luardb.get("wwan.0.txctrl.tx_state_chg.time.nit") or ""
    server_data.nit_time = os.date("%Y-%m-%d %H:%M:%S")
