...
local g_ia_dir = "/usr/share/installation_assistant/magpie/"
...
local g_reg_params = nil
local g_signature_data = nil
...
local JsonRequestHandlerSasResponseInfo = class("JsonRequestHandler", turbo.web.RequestHandler)
...
local InstallScreenStatus = class("InstallScreenStatus", turbo.web.RequestHandler)
local InstallCPISignedData = class("InstallCPISignedData", turbo.web.RequestHandler)
local InstallCPISignatureData = class("InstallCPISignatureData", turbo.web.RequestHandler)
local InstallCPISignatureDataStatus = class("InstallCPISignatureDataStatus", turbo.web.RequestHandler)
...
function JsonRequestHandlerRunTtest:delete(url)
    ...
    -- LARK-567 ftp default port 6220, if test fails, cycle between 6220 & 21
    local lastSpeed = function()
        local speed = luardb.get("service.ttest.ftp.0.res.0.speed")
        if speed == nil then
            -- oa rebooted or previous test fail too early before speed rdb is set
            local test_data = g_ttest_result and g_ttest_result.test_data or {}
            local test_done = #test_data == 2 and test_data[1].status == "completed" and test_data[2].status == "completed"
            if test_done then speed = '0' end
        end
        return speed
    end
    local server_info = (luardb.get("owa.service.ttest.ftp.0.server") or ''):split(":")
    local port = server_info[2]
    local new_port
    if not port then
      new_port = "6220"
    elseif port == "6220" or port == "21" then
      local speed = lastSpeed()
      if speed and (speed == '' or speed == '0') then
        new_port = port == "6220" and "21" or "6220"
      end
    end
    if new_port and server_info[1] then
      local ipaddress = server_info[1]
      luardb.set("service.ttest.ftp.0.server", ipaddress..':'..new_port);
      luardb.set("service.ttest.ftp.1.server", ipaddress..':'..new_port);
    end
...
-- encode SAS response info as JSON blob and send back to the client
function JsonRequestHandlerSasResponseInfo:get()
    local info = {}
    local registration = {}
    local grant = {}
    local modem = {}

    registration.request_code = luardb.get("sas.registration.request_error_code") or ""
    registration.response_code = luardb.get("sas.registration.response_code") or ""
    registration.message = luardb.get("sas.registration.response_message") or ""
    registration.data = luardb.get("sas.registration.response_data") or ""
    registration.request_error = luardb.get("sas.registration.request_error_message") or ""

    grant.request_code= luardb.get("sas.grant.0.request_error_code") or ""
    grant.response_code = luardb.get("sas.grant.0.reason") or ""
    grant.message = luardb.get("sas.grant.0.response_message") or ""
    grant.data = luardb.get("sas.grant.0.response_data") or ""
    grant.request_error = luardb.get("sas.grant.0.request_error_message") or ""

    modem.tx = luardb.get("wwan.0.txctrl.tx_state") or "1"
...
function InstallationState:post()
    ...
    if new_state == "scan_connect" or new_state == "normal_operation" or new_state == "data_entry" then
        local old_state = luardb.get("installation.state")
        luardb.set("installation.state", new_state)
        if old_state == "normal_operation" and new_state == "scan_connect" then
          luardb.set("sas.registration.cmd", "")
        end
    end

    if(new_state == "normal_operation") then
        g_reg_params = nil
        g_signature_data = nil
    end
end
...
function SasRegister:post(url)
    ...
    -- register with data signed
    if params.register and not params.sign_data then
        if not g_signature_data then
            self:set_status(400)
            self:write("No valid signature recieved")
            return
        end

        local cpi_signature = escape.json_encode(g_signature_data)
        if not m_cbrs.invoke_registration(g_reg_params, cpi_signature) then
            self:set_status(400)
            self:write("Failed to invoke registration")
        end
        return
    end
    ...
    params.reg_height_type = params.reg_height_type
    ...
    local reg_params = {
        ...
        height_type = params.reg_height_type,
    ...
    -- sign data only
    if params.sign_data and not params.register then
        g_reg_params = reg_params
        g_signature_data = nil

        local response = {}
        response.href = luardb.get("sas.sign_data_app.url") or ""
    ...
    if params.sign_data and params.register then
        local cpi_signature = m_cbrs.generate_cpi_signature_data(reg_params)
        if not m_cbrs.invoke_registration(reg_params, cpi_signature) then
            self:set_status(400)
            self:write("Failed to sign or register")
        end
    else
        self:set_status(400)
...
-- get installation screen status
function InstallScreenStatus:get(url)
    -- disable cache
    self:set_header("Cache-Control", "no-cache, no-store, must-revalidate")
    self:set_header("Pragma", "no-cache")
    self:set_header("Expires", "0")

    local installStep = luardb.get("installation.state")

    if installStep == "data_entry" then
        installStep = "Data Entry"
    elseif installStep == "scan_connect" then
        installStep = "Network Scan"
    elseif installStep == "normal_operation" then
        if g_reg_params == nil then
            installStep = "CBRS Registration: Verification"
        else
            if g_signature_data == nil then
                installStep = "CBRS Registration: Ready to Sign"
            else
                installStep = "CBRS Registration: Ready to Register"
            end
        end
    end

    self:write({installStep=installStep})
end

-- get sign data
function InstallCPISignedData:get(url)
    -- disable cache
    self:set_header("Cache-Control", "no-cache, no-store, must-revalidate")
    self:set_header("Pragma", "no-cache")
    self:set_header("Expires", "0")
    ...
    if g_reg_params == nil then
        self:set_status(404)
        self:write("Data does not exist")
        return;
    end

    local sign_data = {}
    local install_parameter = {}
    local installer_data = {}

    install_parameter.latitude = g_reg_params.latitude
    install_parameter.longitude = g_reg_params.longitude
    install_parameter.height = g_reg_params.height
    install_parameter.heightType = g_reg_params.height_type
    install_parameter.indoorDeployment = luardb.get("sas.config.indoorDeployment") == "true"
    install_parameter.antennaAzimuth = g_reg_params.azimuth
    install_parameter.antennaDowntilt = g_reg_params.downtilt
    install_parameter.antennaGain = tonumber(luardb.get("sas.antenna.gain") or "0")
    install_parameter.eirpCapability = tonumber(luardb.get("sas.antenna.eirp_cap") or "0")
    install_parameter.antennaBeamwidth = tonumber(luardb.get("sas.antenna.beam_width") or "0")
    install_parameter.antennaModel = luardb.get("sas.antenna.model") or ""

    installer_data.cpiId = g_reg_params.cpi_id
    installer_data.cpiName = g_reg_params.cpi_name
    installer_data.installCertificationTime = g_reg_params.install_certification_time

    sign_data.fccId = luardb.get("sas.config.fccid") or ""
    sign_data.cbsdSerialNumber = luardb.get("sas.config.cbsdSerialNumber") or ""

    sign_data.installationParam = install_parameter
    sign_data.professionalInstallerData = installer_data

    self:write(sign_data)
end

-- get the signature data status
function InstallCPISignatureDataStatus:get(url)
    self:set_header("Cache-Control", "no-cache, no-store, must-revalidate")
    self:set_header("Pragma", "no-cache")
    self:set_header("Expires", "0")

    self:write({signature_data_recieved = nill ~= g_signature_data})
end

-- put signature data
function InstallCPISignatureData:put(url)
    -- disable cache
    self:set_header("Cache-Control", "no-cache, no-store, must-revalidate")
    self:set_header("Pragma", "no-cache")
    self:set_header("Expires", "0")
    ...
    if g_reg_params == nil then
        self:set_status(405)
        self:write("Sign data not received")
        return
    end

    local recieved_data = self:get_json(true)
    if recieved_data == nill or recieved_data.cpiSignatureData == nill or
        recieved_data.cpiSignatureData.digitalSignature == nill or
        recieved_data.cpiSignatureData.encodedCpiSignedData == nill or
        recieved_data.cpiSignatureData.protectedHeader == nill then
        self:set_status(400)
        self:write("Wrong data format")
        return
    end

    g_signature_data = recieved_data.cpiSignatureData
...
app = turbo.web.Application:new({
    ...
    {"/sas_response_info/(.*)$", JsonRequestHandlerSasResponseInfo},
    ...
    {"/install/ScreenStatus", InstallScreenStatus },
    {"/install/cpiSignedData", InstallCPISignedData },
    {"/install/cpiSignatureData$", InstallCPISignatureData },
    {"/install/cpiSignatureDataStatus$", InstallCPISignatureDataStatus },
