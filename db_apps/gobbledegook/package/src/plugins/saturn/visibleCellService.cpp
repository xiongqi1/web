...
class ServicePlugin : public CasaPlugin::CasaServicePlugin {
  public:
    ...
    erdb::Rdb rdbSysSo = erdb::Rdb("wwan.0.system_network_status.current_system_so"); // Current system service options.
...
DEFINE_PLUGIN(ServicePlugin, "visibleCellService", "saturn")
...
std::string ServicePlugin::getVisibleCellList() {
    ...
    if (rdbNr5gUp.get(true).toStdString() == "UP" &&
            rdbSysSo.get(true).toStdString().find("5G SA") == std::string::npos) { // In 5G SA, 5G cell info comes from "wwan.0.cell_measurement.cell_list".
                                                                                   //  Don't need to append cell info, here.
