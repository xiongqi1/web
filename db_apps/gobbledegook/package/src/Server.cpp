...
//         .gattCharacteristicBegin("string", "00000002-1E3C-FAD4-74E2-97A033F1BFAA", {"encrypt-read", "write", "notify"})
//
...
#include "CasaServicePluginHandler.h"
...
// Load dynamic library files, which have GATT service hierarchy.
std::vector<CasaPlugin::CasaServicePluginHandler> load_plugins() {
	std::vector<CasaPlugin::CasaServicePluginHandler> plugins;
	const char * plugins_dirname = "/usr/lib/ggk_plugins/";

	DIR *dir;
	struct dirent *ent;
	if ((dir = opendir (plugins_dirname)) != NULL) {
		while ((ent = readdir (dir)) != NULL) {
			if(std::regex_match (ent->d_name, std::regex("[^\\.].*\\.so")))
				plugins.push_back(CasaPlugin::CasaServicePluginHandler(plugins_dirname + std::string(ent->d_name)));
		}
		closedir (dir);
	}
	return plugins;
	...
	GGKServerDataGetter getter, GGKServerDataSetter setter)
{
	...
	enableBondable = true;  // this is only a startup setting as the main program actively controls bonding
	...
	DBusObject &obj = objects.back();

	// Load and push GATT service hierarchy to D-Bus object on the Server.
	auto plugins = load_plugins();
	for (auto ph : plugins) {
		auto plugin = ph.load();
		LogInfo(("Loaded [" + ph.get_ServiceName() + "] service from [" + ph.get_PluginName() + "] plugin").c_str());
		plugin->append_service(obj);
	}

/*	Example copypasta with minimal comments (see original source for verbose documentation)
	.gattServiceBegin("serviceName", "serviceUUID")
		.gattCharacteristicBegin("characteristicName", "characteristicUUID", {"encrypt-read", "notify"}) // see GattService.h for available permissions
			.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				// example for byte value
				//uint8_t batteryLevel = self.getDataValue<uint8_t>("characteristicName", 0);
				//self.methodReturnValue(pInvocation, batteryLevel, true);
				const char *pTextString = self.getDataPointer<const char *>("characteristicUUID", ""); // dataGetter uses 1st parameter as pName
				self.methodReturnValue(pInvocation, pTextString, true);
			})
			.onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				GVariant *pAyBuffer = g_variant_get_child_value(pParameters, 0);
				self.setDataPointer("characteristicName", Utils::stringFromGVariantByteArray(pAyBuffer).c_str());
				self.callOnUpdatedValue(pConnection, pUserData);
				self.methodReturnVariant(pInvocation, NULL);
			})
			.onUpdatedValue(CHARACTERISTIC_UPDATED_VALUE_CALLBACK_LAMBDA
			{
				const char *pTextString = self.getDataPointer<const char *>("characteristicName", "");
			...
			.gattDescriptorBegin("description", "2901", {"encrypt-read"})
				.onReadValue(DESCRIPTOR_METHOD_CALLBACK_LAMBDA
				{
					const char *pDescription = "characteristicDescription";
