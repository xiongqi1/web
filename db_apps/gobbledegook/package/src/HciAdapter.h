...
public:
	...
	struct DeviceConnectedEvent
	{
		...
		{
			std::string text = "";
			text += "> DeviceConnected event: Address [" + Utils::bluetoothAddressString(address) + "]";
			return text;
		}
	} __attribute__((packed));
	...
	struct DeviceDisconnectedEvent
	{
		...
		std::string simplifiedDebugText()
		{
			...
			estd::DefValueMap<int, std::string> enumAddrType = {
				{0, "BR/EDR"},
				{1, "LE Public"},
				{2, "LE Random"}
			};
			estd::DefValueMap<int, std::string> enumReason = {
				{0, "Unspecified"},
