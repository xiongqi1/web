	...
	while (ggkGetServerRunState() <= ERunning && hciSocket.isConnected())
	{
		...
		switch(eventCode)
		{
			...
			case Mgmt::EDeviceConnectedEvent:
			{
				...
				log(LOG_ERR, "%s", event.simplifiedDebugText().c_str());
				break;
			}
			// Command status event
			...
			{
				if (!CasaDbus::niceMode)
				{
					Logger::error("User isn't paired and is trying to access GATT data.  Removing pairing record!");
					CasaDbus::checkConnectionsForPairing(true);
				}
				else
				{
					Logger::error("Auth failed - user not paired");
				}
			break;
			}
			// Unsupported
...
bool HciAdapter::sendCommand(HciHeader &request)
{
    ...
    Logger::debug(SSTR << "  + SendCommand Sending request " << request.code);
...
{
    Logger::debug(SSTR << "  + Setting command response " << commandCode);
