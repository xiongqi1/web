...
void ggkCheckDasBoot(void)
{
    ...
    {
        ggk::Logger::warn("*** DAS BOOT!! ***");
        HciAdapter::getInstance().dasBoot = false;
        
        ggk::Mgmt mgmt;
        ggk::Logger::warn("Powering off");
        while (!mgmt.setPowered(false))
        {
            ggk::Logger::error("Error turning off the power!"); 
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }
        std::this_thread::sleep_for(std::chrono::seconds(5));
        ggk::Logger::warn("Power back on");
        if (!mgmt.setPowered(true))
        {
            ggk::Logger::error("Error turning on the power!");
            std::this_thread::sleep_for(std::chrono::seconds(1));
        }
    }
}
...
{
    if (state) ggk::Logger::debug("Setting bonding state on");
    else ggk::Logger::debug("Setting bonding state off");
    ggk::Mgmt mgmt;
    if (!mgmt.setBondable(state))
    {
        ggk::Logger::error("Error setting bonding flag!");
        ggkTriggerShutdown();
    }
}
...
void ggkSetConnectable(bool state)
{
    ...
    {
        ggk::Logger::error("Error setting connectable flag!");
        ggkTriggerShutdown();
    }
}
