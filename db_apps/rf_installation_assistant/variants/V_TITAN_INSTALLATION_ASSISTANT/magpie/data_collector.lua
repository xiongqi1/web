...
-- g_user_selected_rsrp_pass array, which can be between 0 and 9 elements long.
...
-- Is the cell sector id one of sector ids entered by the user
...
local function get_registration_status()
    ...
    local net_reg_state = luardb.get("wwan.0.system_network_status.reg_stat")
    ...
    -- Use network registration status in non-CBRS band
    if cbrs_band_mode == "0" then
        if net_reg_state == "1" then
            result = "Registered"
        else
