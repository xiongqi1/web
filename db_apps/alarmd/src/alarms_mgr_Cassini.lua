...
local rpc = rdbrpcclient:new(g_alarm_conf.alarmEndpoint);
...
local function set_alarm_on(subsys, message)
	...
	local id = rpc:invoke('raise', { ['subsys'] = subsys, ['message'] = message }, g_alarm_conf.SEND_ALARMS_EVENT_TIMEOUT)
...
local function _set_alarm_clear(id)
	...
	local result  = rpc:invoke('clear', { ['id'] = id }, g_alarm_conf.SEND_ALARMS_EVENT_TIMEOUT)
...
local function list_alarms()
	local alarms = rdbobject.getClass(g_alarm_conf.alarmClass)
...
function flush_alarms()
	...
	for i, alarm in ipairs(ids) do
		...
		local ret = rpc:invoke('delete', { ['id'] = alarms:getId(alarm) }, g_alarm_conf.SEND_ALARMS_EVENT_TIMEOUT)
		--print("delete after");
...
local function clear_condition_alarm(name)
	...
	local class = rdbobject.getClass(g_alarm_conf.alarmClass)
	local instances = class:getByProperty('subsys', name)

	for _, obj in ipairs(g_alarm_source) do
		if obj.type == g_alarm_conf.MESSGAGE_TYPE_CONDITION and obj.subsys == name then
	...
	if event.EndOfCooldown < currentTime then
		if #event.TriggerTbl >= config.threshold then
			...
			for _, element in ipairs(g_alarm_source) do
				if element.type == g_alarm_conf.MESSGAGE_TYPE_CONDITION and element.subsys == name then
					obj = element
					break
				end
			end
...
function init_conf_generic (name)
	...
	local class = rdbobject.getClass(g_alarm_conf.alarmCnfClass)
	...
	for _, inst in ipairs(instances) do
		...
		local prefix = g_alarm_conf.alarmCnfClass .. '.' .. id .. '.'
...
function update_conf_counter_generic (name)
	if debug > 0 then print("update_conf_counter_generic (".. name ..")"); end
...
-- Load definition of g_alarm_source from external module (if present) or use default
pcall(require, 'alarms_source')

g_alarm_source = g_alarm_source or
{
        {
                type=g_alarm_conf.MESSGAGE_TYPE_CONDITION,
...
local function rdb_alarm_changed(key, val)
	...
	for name, obj in pairs(g_alarm_source) do
		...
		if obj.rdb == key then
			...
			if obj.type == g_alarm_conf.MESSGAGE_TYPE_BOOL then
				set_bool_alarm(obj, val);
			elseif obj.type == g_alarm_conf.MESSGAGE_TYPE_MSG2 then
				set_msg2_alarm(obj, val);

			elseif obj.type == g_alarm_conf.MESSGAGE_TYPE_MSGRDB then
				set_msgrdb_alarm(obj, val);

			elseif obj.type == g_alarm_conf.MESSGAGE_TYPE_RDB2 then
				set_rdb2_alarm(obj, val);

			elseif obj.type == g_alarm_conf.MESSGAGE_TYPE_CONDITION then
				set_condition_alarm(obj, val);

			--elseif obj.type == g_alarm_conf.MESSGAGE_TYPE_DYNAMIC then
...
local function cleanup_alarms(current_time)
	...
	if alarms_obj.cleared_items_count >0 then
		...
		for i, alarm in ipairs(ids) do
			...
			if t  then
				if (t + g_alarm_conf.DELETE_ALARMS_EVENT_TIMEOUT) < current_time then
					...
					for _, obj in pairs(g_alarm_source) do
						...
						--if obj.type == g_alarm_conf.MESSGAGE_TYPE_DYNAMIC then
...
--[[
	* fields of g_alarm_conf.alarmCnfClass object
 -- name
 -- counter
 -- threshold
 -- period
...
local function initialize()
	...
	init_ConfigObject(g_alarm_conf.alarmCnfClass);
	flush_alarms();
...
while( true) do
	...
	luardb.wait(g_alarm_conf.POLL_TIMEOUT);
...
--]]
	...
	for name, rule in pairs(g_rules) do
		if rule.active then
	...
	if (current_time - g_alarm_conf.start_time ) > g_alarm_conf.DELETE_ALARMS_EVENT_TIMEOUT then
		g_alarm_conf.start_time = current_time;
