...
int CyBtldr_FromAscii(unsigned int bufSize, char* buffer, unsigned short* rowSize, unsigned char* rowData)
{
    unsigned short i;
...
int CyBtldr_ParseHeader(unsigned int bufSize, char* buffer, unsigned long* siliconId, unsigned char* siliconRev, CyBtldr_ChecksumType* chksum)
{
    ...
    if (CYRET_SUCCESS == err)
    {
        if (rowSize >= LENGTH_CHKSUM)
            *chksum = (CyBtldr_ChecksumType)rowData[5];
...
int CyBtldr_ParseRowData(unsigned int bufSize, char* buffer, unsigned char* arrayId, unsigned short* rowNum, unsigned char* rowData, unsigned short* size, unsigned char* checksum)
{
