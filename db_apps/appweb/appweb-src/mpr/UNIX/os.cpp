...
#endif
//	syslog(LOG_ERR,"cmdCompleted-1:%d==%d---%d ",signo,SIGCHLD, info);
	saveErrno = errno;
		...
		if (code == CLD_EXITED || code == CLD_KILLED) {
			...
			do {
				...
				if (rc > 0) {
					...
					pid = -1;/* Keep going to reap any other children */
				} else if (errno == EINTR) {
					rc = 1;
				}
			} while (rc > 0);
	...
	else if( chldpid )
	{
		do {
			errno = 0;
			rc = waitpid(chldpid, &status, 0);
			if (rc > 0) {
				status = WEXITSTATUS(status);
				mprGetMpr()->cmdService->processStatus(rc, status);
				chldpid = -1; /* Keep going to reap any other children */
			} else if (errno == EINTR) {
				rc = 1;
			}
//syslog(LOG_ERR,"cmdCompleted-3 chldpid=%d status=%d", chldpid, status);
		} while (rc > 0);
		chldpid = 0;
	}
...
rescan:
	...
	/*
	Iwo: Sometimes, grandchildren die and send SIGCHILD. Since
	we don't know the PIDs, they fill up our pid array. Eventually
	all CGI processing hangs.
	At this point, we find one set PID. Further down, if we
	find that PID in the command list, all is good. Otherwise it gets deleted.
	*/
	for (i = last_idx; i < MPR_CMD_REAP_MAX; i++) {
		if (children[i].pid > 1) {
			lone_pid = children[i].pid;
			lone_idx = i;
			break;
		}
	}
	last_idx = (i<MPR_CMD_REAP_MAX)?i:0;
	...
	while (cp) {
		...
		/* PID is not a grandchild */
		if (lone_pid == pid) lone_pid=0;

		//
		//	Pid will be set when the vfork() system call returns
		//
		if (pid) {
			// poll SIGCHILD - workaround for any broken kernel that does not give us SIG_CHILD - Bovine and Platypus2 for now
			{
				pid_t pid_wait;
				int status;
				...
				if (pid_wait > 0) {
					status = WEXITSTATUS(status);
					mprGetMpr()->cmdService->processStatus(pid_wait, status);
				}
			}
	...
	if(lone_pid) {
		/* Yep, PID was a grandchild, removing it. */
		children[lone_idx].pid = 1;
		last_idx=0;
	}
	...
	void *fnData, int userFlags)
{
	...
	//
	pid = vfork();
//syslog(LOG_ERR,"MprCmd-pid:%d",pid);
	if( pid )
		chldpid = pid;
	if (pid < 0) {
