...
void Mpr::serviceEvents(bool loopOnce, int maxTimeout)
{
	...
	do {
...
#ifdef WORKAROUND_FROZEN_BREAKOUT
		/*
		
		** Workaround for dead breakout UDP socket **
		
		This problem occationally happens and it is seen so far only in Platypus2 platform.
		TODO: If anyone sees the same problem in any other platform, the workaround also needs to be applied.
		
		Description: 
		select() does not tell when the read buffer is changed in the socket even though it returns a success code or timeout.
		I guess that this may be a kernel-related issue or some driver messes up with the kernel poll table.
		
		repairBreakOutIfNeeded() function actually sends a packet through the breakout socket and checks to see if the pipe is working.
		when the pipe does not work, re-open the breakout UDP socket
		
		This work around also reduces the maximum timeout from 60 seconds to 10 seconds. 10 seconds is a completely random period that is
		shorter than 60 seconds and the user may not possbily complain about (or notice).
		...
		*/
		...
		mpr->selectService->repairBreakOutIfNeeded();
#endif		
