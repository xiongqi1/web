...
int MprSelectService::openBreakoutPort()
{
	...
	for (retries = 0; retries < breakRetries; retries++) {
		...
		if (setsockopt(breakSock,SOL_SOCKET,SO_REUSEADDR,(char*)&one,sizeof(one))<0)
		{
			syslog(LOG_ERR,"Can't set SO_REUSEADDR on socket");
			return -1;
		}
#if !WIN && !VXWORKS
...
int MprSelectService::testBreakOut()
{		
	int c;
	...
	if (count != 1) {
		mprLog(6, log, "appweb: sendto() error in breakout - %s\n", strerror(errno));
		goto err;
	}
	
	FD_ZERO(&rfds);
	FD_SET(breakSock, &rfds);
			
	tv.tv_sec = 0;
	tv.tv_usec = 0;
		
	// select
	ret = select(breakSock+1, &rfds, NULL, NULL, &tv);
			
	// if we miss the packet
	if(ret<=0) {
		mprLog(6, log, "appweb: select() error in breakout - %s\n", strerror(errno));
		goto err;
	}
	
	// read breakout
	len = sizeof(breakAddress);
	rc = recvfrom(breakSock, &c, 1, 0, (struct sockaddr*)&breakAddress, (SocketLenPtr) &len);
	if (rc != 1) {
		mprLog(6, log, "appweb: recvfrom() error in breakout - %s\n", strerror(errno));
		goto err;
	}
...
void MprSelectService::repairBreakOutIfNeeded()
{
	lock();
	...
	// FIXME: this is a ugly work-around
	// There does not seem to be any other way to recover this breakout socket failure in Platypus2.
	// It ssems like this issue is caused by some memory corrption in kernel or in appweb by server-side script
	syslog(LOG_EMERG,"appweb: fatal error (breakout socket failure) - abnormal termination!!!");
	abort();
