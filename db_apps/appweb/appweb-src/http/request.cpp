...
//////////////////////////////////////////////////////////////////////////////
static int set_single( char* name, char* value)
{
	...
	retval = rdb_update_string(s, name, value, 0, ALL_PERM);
	if( retval )
	...
	while (state < MPR_HTTP_DONE && inBuf->getLength() > 0) {
		...
		case MPR_HTTP_HEADER:
			if (nbytes > 1) {				// Always trailing newlines
				...
				/*
				 * NTC: Deny access to some file extensions
				 * File extension denied: *.inc
				 */
				if (!strcmp(url.ext, "inc")) {
...
#ifdef FEATURE_SECURE_APPWEB
		requestError(501, "HEAD request is disabled");
		finishRequest();
		return MPR_ERR_BAD_STATE;
#else
...
#ifdef FEATURE_SECURE_APPWEB
		requestError(501, "TRACE request is disabled");
		finishRequest();
		return MPR_ERR_BAD_STATE;
#else
...
int MaRequest::parseHeader(char *line)
{
...
#if MPR_USE_PIPES
		// ignore the length since we are streaming data and not saving
#else		
	...
	/*
	 * NTC: Read header "Referer"
	 */
	else if (strcmp(key, "REFERER") == 0) {
		mprFree(header.referrer);
		header.referrer = mprStrdup(value);
	}

	/*
	 * NTC: to enforce same-origin policy, if "Referer" and "Host" header are available,
	 * the host in "Referer" must be equal to the value of "Host" header
	 */
	if (header.host && header.referrer) {
		char *ip_host, *ip_referrer, *p, *referrer;

		ip_host = strdup(header.host);
		referrer = strdup(header.referrer);
		if (!ip_host  || !referrer) {
			requestError(MPR_HTTP_INSUFFICIENT_STORAGE, "Low memory");
			finishRequest();
			free(ip_host);
			free(referrer);
			return MPR_ERR_BAD_STATE;
		...
		/* extract host in referrer */
		if (strncmp(referrer, "http://", 7) == 0) {
			ip_referrer = referrer + 7;
		}
		else if (strncmp(referrer, "https://", 8) == 0) {
			ip_referrer = referrer + 8;
		}
		else {
			requestError(400, "Bad URL format");
			finishRequest();
		...
		/* take away path part */
		p = strstr(ip_referrer, "/");
		if (p) {
			*p = 0;
		}
		/* take away authentication part */
		p = strstr(ip_referrer, "@");
		if (p) {
			ip_referrer = p+1;
		}
		...
		/* compare */
		if (strcmp(ip_host, ip_referrer)) {
			requestError(400, "Same-origin policy violated");
			finishRequest();
			free(ip_host);
			free(referrer);
			return MPR_ERR_BAD_ARGS;
...
#ifdef FEATURE_SECURE_APPWEB
			if (terminalHandler == 0) {
				mprSprintf(allowBuf, sizeof(allowBuf), "Allow: OPTIONS");
			} else {
...
void MaRequest::requestError(int code, char *fmt, ...)
{
	...
	mprAllocSprintf(&buf, MPR_HTTP_BUFSIZE, 
		...
		"<BODY><H2>Access Error: %d -- %s</br>%s</H2>\r\n"
		"</BODY></HTML>\r\n",
		getErrorMsg(code), code, getErrorMsg(code), logMsg);
...
void MaRequest::writeHeaders()
{
	...
	/*
	 * NTC: Hide server name and version for security reasons
	 * outputHeader("Server: %s", MPR_HTTP_SERVER_NAME);
	 */

	/*
	 * NTC: Instructing browsers to not load Web UI in embbeded frames
	 * The X-Frame-Options header is described in RFC 7034.
	 ...
	 * Obsoleted by Content-Security-Policy, it is for compatibility with old browsers which are lack of support for Content-Security-Policy.
	 *
	 * Content-Security-Policy is used to declare approved origins of content that browsers should be allowed to load on that website.
	 * Below header enforces that the page can only be displayed in a frame on the same origin as the page itself.
	 * Besides scripts, connect (sources for fetch, XMLHttpRequest, WebSocket, and EventSource connections), images, and css must be on the same origin.
	 * Other resources are prohibited.
	 */
	outputHeader("X-Frame-Options: SAMEORIGIN");
	outputHeader("x-xss-protection: 1; mode=block");
	outputHeader("Content-Security-Policy: default-src 'none'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; connect-src 'self';"
			" img-src 'self'; style-src 'self' 'unsafe-inline'; frame-ancestors 'self'; frame-src 'self'; font-src 'self'; object-src 'self'");
...
#ifdef FEATURE_SECURE_APPWEB
				if (methodFlags & (MPR_HANDLER_OPTIONS)) {
#else
...
#ifdef FEATURE_SECURE_APPWEB
				requestError(MPR_HTTP_NOT_IMPLEMENTED,
					"HTTP method \"%s\" is disabled",
					getMethod());
#else
...
void MaRequest::createEnvironment()
{
	...
	if(*contentLengthStr != '-' )
		set_single( "upload.file_size", contentLengthStr );
...
void MaRequest::createQueryVars(char *buf, int len)
{
	...
	while (keyword != 0) {
		...
		if (*keyword) {
			// For security reasons we cannot allow everything the user sends through
			// Without the following check this command would circumvent our usual checks
			// curl -k -d "/sbin/reboot" "https://<hostname>/cgi-bin/sms.cgi?CMD=SMS_CONF_GET&SESSION_ID=1&sessionid=1"
			bool allowedKeyword = true;
			if (0 == strcmp(keyword,"sessionid")) {
				allowedKeyword = false;
			} else if (0 == strcmp(keyword,"SESSION_ID")) {
				allowedKeyword = false;
			}
			/* NTC: CSRF token */
			else if (0 == strcmp(keyword,"csrfToken")) {
				allowedKeyword = false;
			}
			if (allowedKeyword) {
				//
	...
	bool secure)
{
	...
	/*
	 * NTC: Appending the "httpOnly" value to the Set-Cookie response so the cookie will only be accessible to HTTP requests
	 */
	...
	if (lifetime > 0) {
		...
		mprAllocSprintf(&cookieBuf, MPR_HTTP_MAX_HEADER, 
			"Set-Cookie: %s=%s; httpOnly; path=%s; Expires=%s; %s",
			name, value, path, dateStr, secure ? "secure" : "");

	} else {
		mprAllocSprintf(&cookieBuf, MPR_HTTP_MAX_HEADER, 
			"Set-Cookie: %s=%s; httpOnly; path=%s; %s",
