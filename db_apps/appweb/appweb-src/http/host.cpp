...
#if BLD_FEATURE_SESSION
#include	"rdb_ops.h"
static int report_session_timeout(void)
{
	struct	rdb_session *s;
	int	retval;

	syslog(LOG_ERR,"report_session_timeout()...");
	retval = rdb_open(NULL, &s);

	if(retval)
	{
		syslog(LOG_ERR,"rdb_open failed. %d ( %s )", errno, strerror(errno));
		return -1;
	}

	retval = rdb_update_string(s, "webinterface.session_timeout", "1", 0, DEFAULT_PERM);
	if( retval )
		syslog(LOG_ERR,"rdb_update_string returns %d ( %s )", errno, strerror(errno));
	rdb_close(&s);
	return 0;
}
