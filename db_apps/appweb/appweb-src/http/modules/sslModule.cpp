	...
	MaHost *host, MaAuth *auth, MaDir *dir, MaLocation *location)
{
	...
	if (mprStrCmpAnyCase(key, "SSLCACertificatePath") == 0) {
		...
		while (word) {
			...
			} else if (mprStrCmpAnyCase(word, "TLSv1.1") == 0) {
				protoMask &= ~(MPR_HTTP_PROTO_TLSV1_1 & ~mask);
				protoMask |= (MPR_HTTP_PROTO_TLSV1_1 & mask);

			} else if (mprStrCmpAnyCase(word, "TLSv1.2") == 0) {
				protoMask &= ~(MPR_HTTP_PROTO_TLSV1_2 & ~mask);
				protoMask |= (MPR_HTTP_PROTO_TLSV1_2 & mask);
