...
int MaOpenSslConfig::start()
{
	...
	// SSLv2_server_method was removed in OpenSSL v1.1.0
	...
	#else
	context = SSL_CTX_new((protocols == MPR_HTTP_PROTO_SSLV2) ? SSLv2_server_method() : SSLv23_server_method());
	#endif
	mprAssert(context);
	if (context == 0) {
	...
	if (!(protocols & MPR_HTTP_PROTO_TLSV1_1)) {
		SSL_CTX_set_options(context, SSL_OP_NO_TLSv1_1);
		mprLog(4, "SSL: %s: Disabling TLSv1.1\n", hostName);
	}
	if (!(protocols & MPR_HTTP_PROTO_TLSV1_2)) {
		SSL_CTX_set_options(context, SSL_OP_NO_TLSv1_2);
		mprLog(4, "SSL: %s: Disabling TLSv1.2\n", hostName);
	}
	//
...
static int verifyX509Certificate(int ok, X509_STORE_CTX *xContext)
{
	...
	if (X509_NAME_oneline(X509_get_issuer_name(cert), issuer, 
			sizeof(issuer) - 1) < 0) {
		ok = 0;
	}
