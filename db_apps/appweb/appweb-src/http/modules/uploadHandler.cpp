...
static int set_single( char* name, char* value)
{
	...
	retval = rdb_update_string(s, name, value, 0, DEFAULT_PERM);
	if( retval )
...
static int get_single( char* name, char* value, int len)
{
	...
	retval = rdb_get(s, name, value, &len);
		...
		MaHost * host, MaAuth * auth, MaDir * dir, MaLocation *lp)
{
	...
	if (mprStrCmpAnyCase(key, "FileUploadDir") == 0) {
		...
		/* uploadDir is defined in two classes: MaUploadHandler and MaUploadHandlerService.
		 * Each class refer and copy this value but after reading config file,
		 * FileUploadDir value is copied to MaUploadHandler class only. There is no problem with
		 * HTTP pages but by some unknown reason, for HTTPS pages uploadDir is changed to 
		 * '/tmp' later and uploading firmware fails. To prevent this, changed to copy
		 * FileUploadDir value is copied to MaUploadHandlerService class as well and it may not be
		 * harmful at all. */
...
int MaUploadHandler::run(MaRequest * rq)
{
	...
	if (rq->getSession() == 0) {
		mprLog(5, log, "Upload Authentication Error.\n");
		return 0;
	}
	...
	// Check free memory
	//
	get_single( "upload.file_size", buff, sizeof(buff) );
	if( *buff ) {	
		currentsize = atol(buff);
		if( currentsize > 0 ) {
			get_single( "upload.free_size", buff, sizeof(buff) );
			if( *buff ) {
				if( currentsize > atoll(buff)*1024 ) {
					//rq->requestError(MPR_HTTP_BAD_REQUEST, "The device don't have enough memory to upload the file!\n");
					currentsize = 0;
					...
					while (len > 0) {
						...
						currentsize += len;
						sprintf( buff, "%lld", currentsize);
						set_single( "upload.current_size", buff );
						if (rq->getState() != MPR_HTTP_RUNNING) {
							break;
						}
						len = rq->readPostData(postDataBuf, UPLOAD_BUF_SIZE);
	...
	set_single( "upload.current_size", "-1" );
	if (rq->getState() == MPR_HTTP_RUNNING) {
