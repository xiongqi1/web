--[[
  https://github.com/xiaooloong/lua-resty-iconv/blob/master/lualib/resty/iconv.lua
  https://github.com/xiaooloong/lua-resty-iconv/blob/master/LICENSE
  License: GPL V3

  Example:
  local from_enc_string = "UTF16"
  local to_enc_string = "ASCII"

  local ic = require("iconv")
  local icc, err = ic:new(from_enc_string, to_enc_string, 1024) -- The size at the end is optional
  -- or
  local icc, err = ic:new(from_enc_string, to_enc_string)

  if icc == nil then
    print(err)
    return
  end
  print(string.format("Encoding from %s to '%s' is supported", to_enc_string, from_enc_string))

  local out_str, err = icc:convert(input_data)
  icc:finish()
