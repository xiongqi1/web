...
#--------------------------------------------------------------------------
# check if the number is GDSP number)
#--------------------------------------------------------------------------
gdsp_number=false

is_GDSP_number()
{
	for i in "310000214" "310000202" "8823993560000" "8823903560000"; do
		if [ $i = $1 ]; then
		   gdsp_number=true
			return
		fi
...
find_in_whitelist()
{
	if [ "$USE_WHITELIST" = "0" ]; then
		sms_log "Authorization is disabled, bypass auth check!"
		return
	fi

	if [ "${SENDER}" = "" ]; then
		sms_log "SENDER is null, aborting."
		WL_FAIL="1"
		return
	fi

	ROAMING_SIM=`rdb_get manualroam.custom_roam_simcard`
	...
	while [ "$WL_IDX" -le "$MAX_WL_TX_IDX" ]; do
		...
		is_GDSP_number "$SENDER"
		if [ "$ROAMING_SIM" != "1" -a $gdsp_number = true ]; then
			sms_log "not a roaming SIM, GPSP number is denied"
			let "WL_IDX+=1"
			break;
		fi
	...
	if [ "$WL_FOUND" = "FALSE" ]; then
		sms_log "Can't find '$SENDER' in whitelist, aborting."
		WL_FAIL="1"
		return
	fi
}
...
send_error_notification()
{
	if [ "$ENABLE_ERROR_NOTI" = "0" ]; then
		sms_log "SMS error notification is OFF, do not send error notification."
		return
	fi
	...
	if [ "$DEST_NO" = "" ]; then
		sms_log "Destination of SMS error notification is NULL, cannot send error notification!"
		return
	fi
	sms_log "record SMS error notification: $1"
	if [ "$1" = "READ ONLY" ]; then
...
check_password_match()
{
    if [ "$V_PRODUCT" = "ntc_220" ]; then
        ...
        state=$(/sbin/check_passwd "$pass" "$1")
        if [ "$state" != "good" ]; then
...
#--------------------------------------------------------------------------
# Report success or failure back to the sender
#
# $1 : command, $2 : variable/command, $3 : command result
# $4 : if "NOW", send right now, else send later for multiple reply
# $5 : an error flag.  Non-empty/non-zero means a fault has occurred.
#
# If $3 starts with the text "Parameter ERROR" then we treat the response
# as errored.
#
# If there's an error we report that to the sender, otherwise we tell the
# sender their operation was "Successful" and include a few more details.
#--------------------------------------------------------------------------
send_result_and_ack()
{
	...
	Parameter?ERROR*)
		error_condition=true
		param_error_condition=true
		;;
	*)
		param_error_condition=false
		;;
	esac
	...
	if [ "$1" = "get" ]; then
		...
		if [ "$ENABLE_SET_CMD_ACK" = "1" -o "${2}" = "zerosms" ]; then
			...
			# do not use a successful message if we have an error
			if $error_condition; then
				if $param_error_condition; then
					REPLY_SMS="${REPLY_SMS}\"${3}\";"
				elif [ "$3" = "" ]; then
					REPLY_SMS="${REPLY_SMS}\"ERROR: ${1} ${2} command has failed to apply\";"
				else
					REPLY_SMS="${REPLY_SMS}\"${1} ${2}=${3}\";"
				fi
			else
				if [ "$DB_VAR" = "apn" -a "${3}" = "0" ]; then
					REPLY_SMS="${REPLY_SMS}\"Successfully set ${2} to auto\";"
				elif [ "$DB_VAR" = "apn" -a "${3}" = "" ]; then
					REPLY_SMS="${REPLY_SMS}\"Successfully set ${2} to Blank\";"
				elif [ "$DB_VAR" = "password" -a "${3}" = "" ]; then
					REPLY_SMS="${REPLY_SMS}\"Successfully set ${2} to null\";"
				elif [ "${2}" = "zerosms" ]; then
		...
		if $error_condition; then
			if [ "$3" = "" ]; then
				REPLY_SMS="${REPLY_SMS}\"ERROR: ${2} command has failed to apply"
			else
				REPLY_SMS="${REPLY_SMS}\"${3}"
			fi
		else
		...
		if [ "$3" = "" ]; then
			...
			REPLY_SMS="${REPLY_SMS}\"Executed command ${2}:${3}\";"
		fi
...
process_predefined_db_command()
{
	...
	if [ "$1" = "get" ] && [ "$DB_VAR" = "profile" ]; then
		...
		if [ "$DB_APN" = "0" ]; then
			ROAMING_SIM=`rdb_get manualroam.custom_roam_simcard`
			if [ "$ROAMING_SIM" = "1" ]; then
				REPLY_SMS="Auto APN does not apply for roaming SIM cards."
				send_error_notification " " "NOW"
				return 255
			else
				rdb_set link.profile.$PF_NO.autoapn 1
			fi
		else
			rdb_set link.profile.$PF_NO.autoapn 0
	...
	## index 1 : apn(x) cmd
		...
		AUTOAPN=`rdb_get link.profile.$PF_NO.autoapn`
		if [ "$AUTOAPN" = "1" ]; then
	...
	##                          start time, end time, rx, tx
		...
		if [ "$atmgr_ready" != "ready" ]; then		# for platform using cnsmgr only
			if [ "$V_MULTIPLE_WWAN_PROFILES" = "y" ]; then
				USG_STR=`rdb_get link.profile.$PF_NO.usage_current`
			else
			...
			USG_STR=`rdb_get link.profile.$PF_NO.status`
			export WWAN_3G_IF=`rdb_get link.profile.$PF_NO.interface`
	...
	## index 9 : wanuptime cmd, Read Only
		...
		if [ "$atmgr_ready" != "ready" ]; then		# for platform using cnsmgr only
			if [ "$V_MULTIPLE_WWAN_PROFILES" = "y" ]; then
				USG_STR=`rdb_get link.profile.$PF_NO.usage_current`
				if [ "$USG_STR" != "wwan down" ]; then
					START_T=`echo "$USG_STR" | awk -F "," '{print $1}'`
					START_T=`echo $START_T | sed 's/\..*//g'`
					END_T=`echo "$USG_STR" | awk -F "," '{print $2}'`
			...
			if [ "$USG_STR" != "up" ]; then
				...
				if [ "$V_MULTIPLE_WWAN_PROFILES" = "y" ]; then
					START_T=`rdb_get link.profile.1.usage_current | awk '{FS=",";print $1}'`
					END_T=`rdb_get link.profile.1.usage_current | awk '{FS=",";print $1}'`
...
process_predefined_exe_command()
{
	...
	## command 3 : disconnect 3G connection
		...
		rdb_set link.profile.$PF_NO.enable 0
		# checking and setup defaul troute
		if [ "`rdb_get link.profile.$PF_NO.defaultroute`" = "1" ]; then
			for i in 1 2 3 4 5 6; do
				if [ "`rdb_get link.profile.$i.enable`" = "1" ]; then
					rdb_set link.profile.$PF_NO.defaultroute 0
					rdb_set link.profile.$i.defaultroute 1
					return 1
				fi
			done
		fi
	...
	## command 4 : connect 3G connection
		...
		sms_log "call pdpup"
		find_total_active_profile
	#	sms_log "find_total_active_profile=$TOTAL_EN"
		find_default_profile
		MAX_ENABLED_PROFILE=`rdb_get wwan.0.max_sub_if`
		#sms_log "MAX_ENABLED_PROFILE=$MAX_ENABLED_PROFILE"
		if [ $TOTAL_EN -ge $MAX_ENABLED_PROFILE ]; then
			REPLY_SMS="pdpup command has failed! (The maximum number of enabled profiles has been exceeded)"
			send_error_notification " " "NOW"
			return 255
			return 1
		fi
		...
		if [ "$TOTAL_EN" = "0" ]; then
			for i in 1 2 3 4 5 6; do
				rdb_set link.profile.$i.defaultroute 0
			done
			rdb_set link.profile.$PF_NO.defaultroute 1
		elif [ "`rdb_get link.profile.$PF_NO.enable`" = "1" ]; then
			#sms_log "Profile $PF_NO is already activated. No need to run pdpup!"
			REPLY_SMS="Profile $PF_NO is already activated. No need to run pdpup!"
			send_error_notification " " "NOW"
			return 255
		fi
		sms_log "activate profile $PF_NO"
		rdb_set link.profile.$PF_NO.enable 1
	fi
	return 1
}
...
while [ "${CMD_LINE}" != "" ]; do
	...
	if test -n "`echo ${SUB_CMD} | grep -i "\<execute\>" | grep -v "grep"`"; then
		...
		sms_log "Cannot find command in sub cmd line!"
		continue
	fi
	...
	if [ "$extra_commands" != "none" ] && [ "$extra_commands" != "" ]; then
		eval $extra_commands
		send_result_and_ack "$extra_mode" "$extra_command" "$extra_reply_sms" "" "$extra_error"
		continue
	else
