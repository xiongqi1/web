...
-- get extra certificate attributes from a certificate file, that fucntion getCertAttrs does not provide
-- @param cert input certificate file name
-- @ret table containing attribute values
local function getExtraCertAttrs(cert)
    local cmd = string.format('openssl x509 -noout -text -in %s/%s 2>/dev/null', certsDir, cert)
    local cmdOut = Daemon.readCommandOutput(cmd)
    local res = {}
    if cmdOut and cmdOut ~= "" then
        res["signature_algorithm"] = cmdOut:match("%c+%s*Signature Algorithm: ([%w%s]+)%c")
    end
    return res
end
    ...
    for filename in lfs.dir(certsDir) do
        ...
        if attr and attr.mode == 'file' then
            ...
            if output and output ~= '' then
                ...
                local extraAttrs = getExtraCertAttrs(filename)
                for attrKey, attrValue in pairs(extraAttrs) do
                    certTbl[filename][attrKey] = attrValue
                end
            end
...
return {
    ...
    [subRoot .. 'Certificate.*.SignatureAlgorithm'] = {
        get = function(node, name)
            local obj = getCertsObject(node.parent)
            return 0, obj['signature_algorithm'] or ''
        end,
    },
}
