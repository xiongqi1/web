--[[
This script handles parameters for system log configurations under Device.DeviceInfo.VendorLogFile.1.[X_<VENDOR>_XXX|MaximumSize]
...
local function is_large_logfile()
	local rdbName = "service.syslog.option.largelogfile"
	local result = luardb.get(rdbName)
	return result == '1'
end
...
return {
	...
	[subRoot .. '.' .. xVendorPrefix .. '_MaximumSizeInKB'] = {
		...
		set = function(node, name, value)
			local persistent = is_persistent_log()
			local largeLogfile = is_large_logfile()
			local lowerBond = largeLogfile and 10000 or 500
			local upperBond = largeLogfile and 200000 or 5000
			local setVal = tonumber(value)
			if persistent then
				if not setVal or setVal < lowerBond or setVal > upperBond then
