...
local function set_trigflag(idx, prefix)
	Logger.log(logSubsystem, 'debug', 'set trigflag #' .. idx)
	if prefix == g_rdbPrefix_lpo then
		g_lpo_trigflag[idx] = true
	else
...
local function write_trigflag()
	...
	for idx,_ in pairs(g_lpo_trigflag) do
		local rdbKey = g_rdbPrefix_lpo .. idx .. '.trigger'
		Logger.log(logSubsystem, 'debug', 'write ' .. rdbKey)
		luardb.set(rdbKey, '1')
	end
end
...
local function save_if_changed(param, idx, newVal, flag, prefix)
	local prefix = prefix and prefix or g_rdbPrefix_lp
	local rdbKey = prefix .. idx .. param
...
local ExtendedCellularHandler = {
	get = function(node, name)
		...
		if node.name == 'IPv4Status' then
			local status = luardb.get(g_rdbPrefix_lpo .. idx .. '.status_ipv4') or ''
			return 0, status
		end

		if node.name == 'IPv6Status' then
			local status = luardb.get(g_rdbPrefix_lpo .. idx .. '.status_ipv6') or ''
			return 0, status
		end
...
return {
	...
	[subRoot .. 'Interface.*.*'] = {
		get = function(node, name)
			...
			if node.name == 'CurrentAccessTechnology' then
				...
				-- Bell Requirement for 5G indicator. Bell decided to use GSMA recommendation
				-- Config D to reduce the variability of the icon flickering between 5G and LTE
				-- during the roll out.
				--
				-- 	State                                                                                   Config. D
				--  --------------------------------------------------------------------------------------------------
				--	Idle under or connected to LTE cell not supporting NSA                                  LTE
				--	Idle under or connected to LTE cell supporting NSA and no detection of NR coverage      5G
				--	Connected to LTE only under LTE cell supporting NSA and detection of NR coverage        5G
				--	Idle under LTE cell supporting NSA and detection of NR coverage                         5G
				--	Connected to LTE + NR under LTE cell supporting NSA                                     5G
				--	Idle under or connected to NG-RAN while attached to 5GC                                 5G
				local nr5g = luardb.get(g_rdbPrefix_wwan .. idx .. '.system_network_status.endc_avail')
				if nr5g == "1" then
					return 0, "NR"
				end

				--[=[
				-- Valid values are updated in TR-181 v2.14
				-- * GPRS (GSM with GPRS)
				-- * EDGE (GSM with EDGE)
				-- * UMTS
				...
				-- * CDMA2000HRPD
				-- * LTE
				-- * NR (5G New Radio, added in v2.14)
				--]=]
				local ratPairs = {
					["cdma2000 1x"] = "CDMA2000OneX",
					["cdma2000 hrpd (1xev-do)"] = "CDMA2000HRPD",
					["gsm"] = "GPRS",
					["umts"] = "UMTS",
					["lte"] = "LTE",
					["nr5g"] = "NR",
				}
				return 0, ratPairs[rat] or ""
			end
			...
			if node.name == 'RSRP' then
				local rat = luardb.get(g_rdbPrefix_wwan .. idx .. ".system_network_status.service_type") or ""
				local rsrp = nil
				if rat == "lte" then
					rsrp = luardb.get(g_rdbPrefix_wwan .. idx .. ".signal.0.rsrp")
				elseif rat == "nr5g" then
					rsrp = luardb.get(g_rdbPrefix_wwan .. idx .. ".radio_stack.nr5g.rsrp")
				end
				return 0, rsrp and tostring(rsrp) or node.default
			end

			if node.name == 'RSRQ' then
				local rat = luardb.get(g_rdbPrefix_wwan .. idx .. ".system_network_status.service_type") or ""
				local rsrq = nil
				if rat == "lte" then
					rsrq = luardb.get(g_rdbPrefix_wwan .. idx .. ".signal.rsrq")
				elseif rat == "nr5g" then
					rsrq = luardb.get(g_rdbPrefix_wwan .. idx .. ".radio_stack.nr5g.rsrq")
				end
				return 0, rsrq and tostring(rsrq) or node.default
			end
	...
	[subRoot .. 'Interface.*.Stats.*'] = {
		get = function(node, name)
			...
			local ifaces = {}
			for i=1,numAPs do
				local enable = luardb.get(g_rdbPrefix_lpo .. i .. '.enable')
				local status_ipv4 = luardb.get(g_rdbPrefix_lpo .. i .. '.status_ipv4')
				local status_ipv6 = luardb.get(g_rdbPrefix_lpo .. i .. '.status_ipv6')
				local rdbKey = g_rdbPrefix_lpo .. i .. '.ifdev'
	...
	[subRoot .. 'AccessPoint.*.*'] = {
		...
		set = function(node, name, value)
			...
			Logger.log(logSubsystem, 'debug', 'set AP#' .. idx .. ' ' .. node.name .. '=' .. value)
			if node.name == 'Enable' then
				save_if_changed('.enable', idx, value, 'trig', g_rdbPrefix_lpo)
				return 0
			end
