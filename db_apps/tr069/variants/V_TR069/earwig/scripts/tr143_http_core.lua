...
local iface = 'ptm0' -- default interface
...
local function readStatistics(iface, name)
    ...
    dinfo("Stripping vlan interface " .. iface .. " to " .. iface_base)
...
local function record_bytes_received_begin()
    ...
    dinfo('rx_bytes iface= ' .. iface .. ' begin=' .. tostring(diagData.rx_bytes_begin))
...
local function record_bytes_received_end(rx_bytes_body)
    ...
    dinfo('rx_bytes iface= ' .. iface .. ' end=' .. tostring(diagData.rx_bytes_end))
...
local function record_bytes_sent_begin()
    ...
    dinfo('tx_bytes iface= ' .. iface .. ' begin=' .. tostring(diagData.tx_bytes_begin))
...
local function record_bytes_sent_end(tx_bytes_body)
    ...
    dinfo('tx_bytes iface= ' .. iface .. ' end=' .. tostring(diagData.tx_bytes_end))
...
local function trequest(reqt)
    ...
    local ret, err = hostCheck(parsed_url.host)
    if not ret then
        set_DiagnosticsState(err, downUp)
        return
    end
    ...
    if reqt.tos then
        ch:setopt(curl.OPT_SOCKOPTFUNCTION, function(userparams, fd, purpose)
                      local optval = string.char(reqt.tos)
                      return curl.setsockopt(fd, curl.IPPROTO_IP, curl.IP_TOS, optval)
        end)
    end

    ch:setopt(curl.OPT_URL, reqt.url)
...
local function download(url, dscp, intf, startTime)
...
local function upload(url, len, dscp, intf, startTime)
