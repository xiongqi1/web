...
local function ipping(host, reps, timeout, size, dscp, iface, proto)
    ...
    --reset state to '' to skip rdb set in updateRdb()
    diagData.state = ''
    -- update rdb with default values
    updateRdb()

    local ret, err = utils.hostCheck(host, proto)
    if not ret then
        set_DiagnosticsState(err)
        return ret
    end
    host = ret -- use the resolved IP address
