...
-- remove a sample set object id from changedIds table
local function removeChangedId(id)
    id = tonumber(id)
    changedIds[id] = nil
end
    ...
    for _, param in pairs(params) do
        if param.enable == '1' then
            if param.reference and param.reference ~= '' then
                ...
                local node = paramTree:find(param.reference)
                local succ = false
                if node then
                    local status, ret, value = pcall(node.getCWMPValue, node)
...
local function pollPeriodicStats(task)
    ...
    for id, _ in pairs(changedIds) do
...
return {
    [subRoot .. 'SampleSet'] = {
        init = function(node, name)
            ...
            for _, id in ipairs(ids) do
                ...
                -- Reset all records, here.
                -- Otherwise, previous records are available until executing parameterPoll task.
                local obj = sampleSetObjectClass:getById(id)
                clearSampleSet(obj)
                local start_time = tonumber(obj.start_time) or 0
                if start_time > 1440000000 then
                    if obj.enable == '1' then
                        collectTime[id] = start_time + (obj.sample_interval or 3600)
                    else
                        collectTime[id] = nil
                    end
                else
                    -- Need to reset start/end time in parameterPoll task, later,
                    -- because system time is not valid.
                    obj.start_time = 0
                    obj.end_time = 0
                    addChangedId(id)
                end
    ...
    [subRoot .. 'SampleSet.*'] = {
        ...
        delete = function(node, name)
            ...
            local obj = sampleSetObjectClass:getById(id)
            if obj.enable == '1' then
                -- stop parameterPoll from collecting this sample set
                removeChangedId(id)
                collectTime[id] = nil
            end
