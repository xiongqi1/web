...
local function getTriggerSourceRdb(path_name)
    local pathBits = path_name:explode('.')
    return conf.rdb.tr143Prefix .. '.' .. fieldMapping[pathBits[#pathBits-1]] .. '.trigger_src'
end

local function getTriggerSource(path_name)
    return luardb.get(getTriggerSourceRdb(path_name))
end
...
-- validate a value for an enumeration
-- @param value The value to be validated
-- @param enum An array of all possible values
-- @return 0 if validation succeeds; CWMP error code on failure
local function validate_enum(value, enum)
...
local validators = {
    ...
    NumberOfRepetitions = function(value) return validate_int(value, 1, 4294967295) end,
    Timeout = function(value) return validate_int(value, 1, 4294967295) end,
    ...
    ProtocolVersion = function(value) return validate_enum(value, {'Any', 'IPv4', 'IPv6'}) end,
    TestFileLength = function(value) return validate_int(value, 0, 4294967295) end,
}
...
local ipv6enabled = luardb.get("tr069.ipv6_enable")
...
return {
    ...
    [subRoot .. '*'] = {
        init = function(node, name)
            ...
            if node.name == 'IPv4PingSupported' or (node.name == 'IPv6PingSupported' and ipv6enabled == '1') then
                stop_program(program_table.ipping)
                result = start_program(program_table.ipping)
            elseif node.name == 'IPv4DownloadDiagnosticsSupported' or (node.name == 'IPv6DownloadDiagnosticsSupported' and ipv6enabled == '1') then
                stop_program(program_table.download)
                result = start_program(program_table.download)
            elseif node.name == 'IPv4UploadDiagnosticsSupported' or (node.name == 'IPv6UploadDiagnosticsSupported' and ipv6enabled == '1') then
        ...
        get = function(node, name)
            if node.name == 'IPv4PingSupported' or
               ...
               node.name == 'IPv4UploadDiagnosticsSupported' or
               (ipv6enabled == '1' and
                   (node.name == 'IPv6PingSupported' or
                    node.name == 'IPv6DownloadDiagnosticsSupported' or
                    node.name == 'IPv6UploadDiagnosticsSupported')) then
                return 0, '1'
            end
    ...
    [subRoot .. '*.*'] = {
        init = function(node, name)
            ...
            if node.name == 'DiagnosticsState' then
                ...
                local watcher = function(key, value)
                    ...
                    if value then
                        local triggerSrc = getTriggerSource(name)
                        -- if the trigger source is not tr069, ignore it
                        if triggerSrc ~= 'tr069' then
                            dinfo('trigger source: ' .. tostring(triggerSrc) .. ' ignored')
                            return
                        end
        ...
        set = function(node, name, value)
            ...
            if node.name == 'DiagnosticsState' then
                ...
                setDiagState[getTriggerSourceRdb(name)] = 'tr069'
