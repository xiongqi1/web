...
local function createPsTbl(subRoot, numOfEnt)
    ...
    local instId = 1
    local prevNumOfInst = subRoot:countInstanceChildren()
    for line in psFd:lines() do
        repeat -- this is to mimic continue with break
            ...
            local instance = subRoot:getChild(instId)
            if not instance then
                instance = subRoot:createDefaultChild(instId)
            end
            instId = instId + 1
    ...
    for idx=instId, prevNumOfInst do
        local extraInst = subRoot:getChild(idx)
        if extraInst then
            subRoot:deleteChild(extraInst)
        end
    end
