...
local subROOT = conf.topRoot .. '.LANDevice.1.LANHostConfigManagement.'
...
local g_prefixRdb='service.dhcp.static.'
local g_suffixRdb=''
local g_rdbTemplateTrigger = 'service.dhcp.static.trigger'
...
-- [start] for DHCPStaticAddress Object --
get_NumOfRDBInst = function ()
	count = 0
	for i, value in hdlerUtil.traverseRdbVariable{prefix=g_prefixRdb , suffix=g_suffixRdb , startIdx=0} do
...
poller = function (task)
	local currNumOf = get_NumOfRDBInst()
...
addRDBObjInst = function ()
	local currNumOfRdb = get_NumOfRDBInst()

	luardb.set(g_prefixRdb .. currNumOfRdb, ',00:00:00:00:00:00,0.0.0.0,disabled')
	luardb.set(g_prefixRdb .. (currNumOfRdb+1), '')

	g_DHCPReservedAddrList = nil
	getDHCPReservedAddrList()

	return currNumOfRdb + 1
end

delRDBObjInst = function (dataModelObjIdx)
	...
	local currTbl = getDHCPReservedAddrList()

	for i=0, (currNumOf-1) do
		if rdbIndex <= i then
			local nextContent = luardb.get(g_prefixRdb .. (i+1)) or ''
			luardb.set(g_prefixRdb .. i, nextContent)
		end
	end

-- 	luardb.unset(g_prefixRdb .. currNumOf)
-- 	luardb.unset(g_prefixRdb .. (currNumOf-1))

	if currTbl[tonumber(dataModelObjIdx)].status == 'enabled' then
		if client:isTaskQueued('postSession', triggerDHCPStaticAddressList) ~= true then
			client:addTask('postSession', triggerDHCPStaticAddressList, false)
		end
	end
	...
	return get_NumOfRDBInst()
end

getDHCPReservedAddrList = function()
	if g_DHCPReservedAddrList then return g_DHCPReservedAddrList end

	g_DHCPReservedAddrList = {}

	for _, addrInfo in hdlerUtil.traverseRdbVariable{prefix=g_prefixRdb, startIdx=0} do
...
-- service.dhcp.static.{i} [Ex: name,mac_addr,ip_addr,status(enabled|disabled)]
setDHCPStaticAddressList =  function (index, element, value)
	...
	luardb.set(g_prefixRdb .. (index -1), setVal)
...
triggerDHCPStaticAddressList = function ()
...
-- Default Value: 1
-- Available Value: 1|0
-- Involved RDB variable: service.dhcp.enable
	[subROOT .. 'DHCPServerEnable'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luardb.get("service.dhcp.enable");
		...
		set = function(node, name, value)
			...
			local currRelayMode = luardb.get("service.dhcp.relay.0")
			local currDhcpMode =  luardb.get("service.dhcp.enable")

			if currDhcpMode and currDhcpMode == value then return 0 end

			if value == "1" and currRelayMode and currRelayMode == "1" then
				luardb.set("service.dhcp.relay.0", "0")
			end

			luardb.set("service.dhcp.enable", value)
			return 0
		end
	},
...
-- Default Value: 0
-- Available Value: 1|0
-- Involved RDB variable: service.dhcp.relay.0
	[subROOT .. 'DHCPRelay'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luardb.get("service.dhcp.relay.0");
		...
		set = function(node, name, value)
			...
			if value == "1" and currDhcpMode and currDhcpMode == "1" then
				luardb.set("service.dhcp.enable", "0")
			end

			luardb.set("service.dhcp.relay.0", value)
			return 0
		end
	},
...
-- Involved RDB variable: service.dhcp.relay.server.0
	[subROOT .. 'X_NETCOMM_DHCPRelayAddr'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luardb.get("service.dhcp.relay.server.0");
		...
		set = function(node, name, value)
			...
			luardb.set("service.dhcp.relay.server.0", value)
			return 0
		end
	},
...
-- Default Value:
-- Available Value: IP Address
-- Involved RDB variable: service.dhcp.range.0 [Comma-separated min and max addresses- 192.168.1.100,192.168.1.244]
	[subROOT .. 'MinAddress'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luardb.get('service.dhcp.range.0')
...
-- Default Value:
-- Available Value:
-- Involved RDB variable: service.dhcp.static.{i} (Ex: name,mac_addr,ip_addr,enabled|disabled)
...
-- Involved RDB variable: link.profile.0.netmask
	[subROOT .. 'SubnetMask'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
		...
		set = function(node, name, value)
			...
			local currVal = luardb.get('link.profile.0.netmask')
...
-- Involved RDB variable: "service.dhcp.dns1.0" and "service.dhcp.dns2.0"
	[subROOT .. 'DNSServers'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local addrList = {}

			for _, address in hdlerUtil.traverseRdbVariable{prefix='service.dhcp.dns', suffix='.0', startIdx=1} do
		...
		set = function(node, name, value)
			...
			for i=1, numOfDNS do
				...
				local currVal = luardb.get('service.dhcp.dns' .. i .. '.0')
				if currVal ~= dns_addr then
					luardb.set ('service.dhcp.dns' .. i .. '.0', dns_addr)
				end
...
-- Default Value: ""
-- Available Value:
-- Involved RDB variable: service.dhcp.suffix.0
	[subROOT .. 'DomainName'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			local retVal = luardb.get('service.dhcp.suffix.0')
			return 0, retVal or ''
		end,
		set = function(node, name, value)
			if not value then return CWMP.Error.InvalidParameterValue end

			local currVal = luardb.get('service.dhcp.suffix.0')
...
-- Default Value:
-- Available Value: IP Address
-- Involved RDB variable: link.profile.0.address
	[subROOT .. 'IPRouters'] = {
		init = function(node, name, value) return 0 end,
		get = function(node, name)
			return 0, luardb.get('link.profile.0.address') or ''
		end,
		set = function(node, name, value)
			if not value then return CWMP.Error.InvalidParameterValue end
			...
			local currVal = luardb.get('link.profile.0.address')
			
			if currVal ~= setVal then
				luardb.set('link.profile.0.address', setVal)
			end
			return 0
		end
	},
...
-- Default Value: 86400
-- Available Value:
-- Involved RDB variable: service.dhcp.lease.0
	[subROOT .. 'DHCPLeaseTime'] = {
		...
		set = function(node, name, value)
			...
			local currVal = luardb.get('service.dhcp.lease.0')
			currVal = string.trim(currVal)
			if currVal == value then return 0 end
			luardb.set('service.dhcp.lease.0', value)
			return 0
		end
	},
...
-- Involved RDB variable:
	[subROOT .. 'DHCPStaticAddress'] = {
		init = function(node, name, value)
			...
			local numOfInst = get_NumOfRDBInst()
		...
		create = function(node, name)
			local dataModelIdx = addRDBObjInst()
...
-- Default Value:
-- Available Value:
-- Involved RDB variable: service.dhcp.static.{0}
-- element of g_DHCPReservedAddrList[i] --> {name, mac_addr, ip_addr, status(enabled|disabled)}
	[subROOT .. 'DHCPStaticAddress.*.*'] = {
		...
		get = function(node, name)
			...
			local retVal = ''
			local infoTbl = getDHCPReservedAddrList()
		...
		set = function(node, name, value)
			...
			if paramName == 'Enable' then	-- status
				...
				if not infoTbl[dataModelIdx].status or infoTbl[dataModelIdx].status ~= setValue then
					setDHCPStaticAddressList(dataModelIdx, 'status', setValue);
				end
			elseif paramName == 'Chaddr' then	-- mac address
				...
				if not infoTbl[dataModelIdx].mac_addr or infoTbl[dataModelIdx].mac_addr ~= setValue then
					setDHCPStaticAddressList(dataModelIdx, 'mac_addr', setValue);
				end
			elseif paramName == 'Yiaddr' then	-- ip address
				setValue = value
				...
				if not infoTbl[dataModelIdx].ip_addr or  infoTbl[dataModelIdx].ip_addr ~= setValue then
					setDHCPStaticAddressList(dataModelIdx, 'ip_addr', setValue);
				end
			elseif paramName == 'X_NETCOMM_ComputerName' then	-- name
				setValue = value

				if not infoTbl[dataModelIdx].name or infoTbl[dataModelIdx].name ~= setValue then
					setDHCPStaticAddressList(dataModelIdx, 'name', setValue);
				end
			else
				error('Dunno how to handle ' .. name)
			end
