...
function setDiagnosticsState()
	local index
	local kvpair
	for _, kvpair in ipairs(setDiagState) do 
		dinfo( 'TR143: luardb set in postSession - key = ', kvpair[1], ' value to be set = ',kvpair[2])
		luardb.set(kvpair[1],kvpair[2])
	end

        for index = 1, #setDiagState, 1 do
                table.remove(setDiagState, 1)
        end
...
return {
	...
	['**.Diagnostics.Download|Upload|UDPEcho.*.TC1|TC4.*'] = {
		init = function(node, name, value)
			--dinfo( 'TR143: |Download|Upload|UDPEcho|.*.TC1|TC4.* init name = ', name)
	...
	['**.Diagnostics.Download|Upload|UDPEcho.*.*'] = {
		...
		set = function(node, name, value)
			...
			if node.name == 'DiagnosticsState' then
				...
				local stateValue = {getRDBKey(node, name), value}
				table.insert(setDiagState,stateValue)	

				if client:isTaskQueued('postSession', setDiagnosticsState) ~= true then
