...
--
local function deleteMpls(trunk_vid)
  if trunk_vid > 9 then
...
--  end
 ...
 local frame_size = forcenumber(luardb.get('unid.max_frame_size'))
  if frame_size >= 512 and frame_size <= 1944 then
	frame_size = frame_size -8; -- delete crc and vlan-id
...
--]]
	...
	local ecode = { ['EXEC'] = 'ifconfig ' .. conf.wntd.grePrefix ..trunk_vid..' mtu '.. gre_framesize }
	addVlanQ(ecode)
  end
...
local function makeAvc(ind, avc)
  ...
  local unidMode = unid:getMapTag()
  avc['unidMode'] = unidMode

  if unidMode == 'default' or unidMode == 'transparent' then
    avc['vid'] = unidId
  elseif unidMode == 'tagged' then
    ...
    if complain(trunk_vid, 'makeAvc no free vlans avc:'..ind..' uni:'..avc.unid..' vid:'..avc.vid, avc) then
      ...
      complain(false, 'makeAvc unid mode not correct avc:'..ind..' uni:'..avc.unid..' unidMode:'..unidMode, avc)
      return nil
  end
...
local function setMaxFrSize(update_gre)
  ...
  framesize = framesize -4 --wombat not count vlan-id as packet size
  local ccode = { ['CONFIG'] = 'echo ' .. cmd .. framesize }
  addVlanQ(ccode)
  local wntd_framesize = framesize -4 -- remove crc
  ...
  dinfo('mtu wntd='..wntd_mtu, 'gre='..gre_framesize, 'rf='..rf_framesize)
  ...
  if update_gre then
    for _, avc in pairs(avc_tab) do
      if forcenumber(avc.trunk_vid) ~= 0 then
        local ecode = { ['EXEC'] = 'ifconfig ' .. conf.wntd.grePrefix .. avc.trunk_vid .. ' mtu '.. gre_framesize }
        addVlanQ(ecode)
      end
    end
  end
  ...
  function dodata()
    while true do
      ...
      if m then
        ...
        collect.set(prefix..'dropped', t[3]);
        collect.commit()
        return dosearch()
      end
    end
