...
function getStatsRollName()
  ...
  local timeDiff = os.difftime(os.time(),logTime)
  ...
  if logFileSize >= fileRollSize or timeDiff >= fileRollPeriod then
    ...
    for _,v in ipairs(filetab) do
      local numtag = string.sub(v,baselen+1)+1	
      if not os.rename(v, catFileName(numtag)) then
        luasyslog.log('LOG_ERR', 'mv '..v..' '..catFileName(numtag)..' failed')
      else
        dinfo('mv '..v..' '..catFileName(numtag)..' done')
      end
    end
    if #filetab >= fileRollCount then
	local lasttag = string.sub(filetab[1],baselen+1)+1
	local lastfile = catFileName(lasttag)
        if (os.execute('rm '..lastfile) ~= 0) then
        	luasyslog.log('LOG_ERR', 'Error deleting last file '..lastfile)
	else
      		dinfo("deleted the last log file "..lastfile)
        end
    end
    logTime = os.time()
...
function checkStatDir()
  dinfo("Checking the sizes of stats files")
  ...
  for _,v in ipairs(statfolder) do
    ...
    file:close()
    if size >= fileRollSize then
      if (os.execute('rm '..v)~= 0) then
        luasyslog.log('LOG_ERR', 'Error deleting large file '..v)
      else
	dinfo("deleted the large log file "..v.." with size "..size)
