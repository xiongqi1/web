...
local function newreport(unid)
  ...
  for k,v in pairs(unid.avg) do
    ...
    v.lastval = val
    v.laststamp = unid.stamp or 0
  end
  unid.startReportStamp = unid.stamp or 0
end
...
local function stats(unid)
  ...
  if unid.stamp - start >= mibReportInterval - 1 then
    report(prefix, unid)
    newreport(unid)
  end
end
