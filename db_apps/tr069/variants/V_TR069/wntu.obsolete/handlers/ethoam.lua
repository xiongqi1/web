...
local rpc  --= rdbrpcclient:new(conf.wntd.dot1agPrefix);
...
local function node_change_notify()
	...
	for id, val in pairs(active_ids) do
		...
		if val and not table.contains(ids, id) then
			...
			--dinfo( 'Rmp: delObject ',  RMP_node:getPath())

			local child_node = RMP_node:getChild(id);
			if child_node then
	...
	for _, id in  pairs(ids) do
		if not active_ids[id] then
		 -- new note
			...
			node.new_id = id;
			local ret, instanceId = RMP_node:createInstance(RMP_node:getPath(true))
			dinfo('addObject ', RMP_node:getPath(), ret, instanceId)
...
--]]
	...
	['**.Dot1ag.Rmp'] = {
			...
			if rpc then
				...
				if check_result(list) then
					for objid, rmpid in string.gmatch(list, "%((%d%d*),%s*(%d%d*)%)") do
						...
						if objid > last_instanceId then
							last_instanceId =	objid
						end
		...
		create = function(node, name)
			dinfo( 'RMP: create instance ', name)

			if rpc then
				last_instanceId = last_instanceId +1;
				-- create the RDB instance

				result = add_rmp(last_instanceId);
				--dinfo2(result)
				-- create parameter tree instance
				if check_result(result) then
					local treeInstance = node:createDefaultChild(last_instanceId)
					treeInstance:recursiveInit()
					--node.instance = instanceId;
					--active_ids[last_instanceId] =1;
					return  0, last_instanceId
				end
