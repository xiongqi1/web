...
return {
	...
	['**.AVC.*.TC1|TC2|TC3|TC4.*'] = {
		init = function(node, name, value)
			-- initialize watcher id on the node object
			node.rdbWatcherId = nil
			-- check RDB state and default if required
			...
			node.rdbWatcherId = rdbevent.onChange(node.key, watcher)
	...
	['**.AVC.*'] = {
		...
		delete = function(node, name)
			...
			node:forAll(function(n)
				if n.rdbWatcherId then
					rdbevent.removeListener(n.rdbWatcherId, true)
				end
			end, true)

			-- then delete parameter tree instance
	...
	['**.AVC.*.*'] = {
		init = function(node, name, value)
			...
			node.rdbWatcherId = rdbevent.onChange(key, watcher)
			-- we don't need to queueChange here
			return 0
		end,
		get = function(node, name)
