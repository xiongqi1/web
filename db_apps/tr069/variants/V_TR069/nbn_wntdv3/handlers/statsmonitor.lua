...
------------------------------------------------
local function getConfigRDB(name)
	local pathBits = name:explode('.')
	local fieldMapping = {
		['SampleInterval'] = 'sampleinterval',
		['ReportSamples'] = 'reportsamples',
		['SampleEnable'] = 'sampleenable',
		['ThroughputSampleInterval'] = 'throughputsampleinterval',
		['DownlinkThroughputBucketEnable'] = 'downlinkthroughputenable',
		['UplinkThroughputBucketEnable'] = 'uplinkthroughputenable',
		['DownlinkThroughputBucket'] = 'downlinkthroughputbucket',
		['UplinkThroughputBucket'] = 'uplinkthroughputbucket',
	}
	...
	return  conf.stats.statsMonitoringConfigurationPrefix .. "." .. fieldname;
end
...
local function getThroughputValue(name)
	local pathBits = name:explode('.')
	local fieldMapping = {
		['Bucket1']	= 'xbkt1',
		['Bucket2']	= 'xbkt2',
	...
	local key =pathBits[#pathBits - 1];
	local id = pathBits[#pathBits - 2].."";
	local fieldname = fieldMapping[pathBits[#pathBits]];
	if avc_tab[id] then
		if key == 'UplinkThroughput' then
			return avc_tab[id]['t' .. fieldname] -- uplink field named txbytes{1-9}
		elseif key == 'DownlinkThroughput' then
			return avc_tab[id]['r' .. fieldname] -- downlink field named rxbytes{1-9}
		end
	end
end
...
local function getRadioStatsValue(name)
	...
	local fieldMapping = {
		...
		['ServingCellEARFCN'] = 'servingcellearfcn',
	}
	if not fieldMapping[pathBits[#pathBits]] then
		error('StatsMonitoring: unknown parameter to map "' .. pathBits[#pathBits] .. '"')
	end
...
local function rdbChangeNotify()
	luardb.set(conf.stats.statsMonitoringConfigurationPrefix .. '.changed', "")
	dinfo('statsmonitoring.configuration changed')
end
...
return {
	...
	['**.StatsMonitoring.Configuration.*'] = {
		init = function(node, name)
			node.value = node.default
			dinfo('StatsMonitoring.Configuration.* : model init')
			return 0
		end,
		get = function(node, name)
			local rdb_val = luardb.get(getConfigRDB(name))
			get_node_value(node, rdb_val);
			return 0, node.value
		end,
		set = function(node, name, value)
			node.value = value
			luardb.set(getConfigRDB(name), value)
			queueChange()
			return 0
		end,
	...
	['**.StatsMonitoring.Sample.Radio.ServingCellID|ServingCellEARFCN'] = {
		init = function(node, name)
			dinfo('Sample.Radio.Serving* : model init')
			return 0
		end,
		get = function(node, name)
	...
	----
	-- Instance throughput Fields
	----
	['**.StatsMonitoring.Sample.AVC.*.UplinkThroughput|DownlinkThroughput.*'] = {
		init = function(node, name, value)
			dinfo('Sample.UplinkThroughput|DownlinkThroughput.*: init '..node:getPath())
			get_node_value(node, getThroughputValue(name))
			return 0
		end,
		get = function(node, name)
			return 0, get_node_value(node, getThroughputValue(name))
		end,
		set = CWMP.Error.ReadOnly,
		unset = CWMP.Error.OK,
		create = CWMP.Error.ReadOnly,
