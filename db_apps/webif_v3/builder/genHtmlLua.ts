...
function genHtmlFile(pageScript) {
  ...
  head += htmlTag("script", {},
    ...
    + "var userGroup = '{{loggedInUserGroup}}';"
    + "var userGroups = userGroup.split(',');"
#else
...
#endif
    // parse query string and store in a dictionary
  ...
  var scriptFiles = [
    ...
    "/js/powerdown.js",
#endif
    "/js/generatedMenuEntries.js",
    "/theme/js/genHeader.js",
    "/theme/js/genFooter.js"
  ];
  ...
  head += genThemeHeadTags(pageData);
...
#ifdef V_WEBIF_SERVER_turbo
  // NOTES: The legacy code in genHtmlLua.js considers to copy *.lua files (upload handlers) to staging/www/cgi-bin/
  // in genHtmlPage function.
  // In new structure, those files could be in www/ directory, which would be installed into staging/www/cgi-bin/
  // if necessary (controlled by V-var).
#else
  // FIXME: if appweb was supported, following code would needs to be rewritten.
  //        For example, is it possible to refactor upload handlers into PageElements?
  //
