...
function getValidateLuaStatement(mem){
  ...
  var luaChecks;
  if (mem.customValidateLuaCheck) {
    luaChecks = mem.validateLua.join("");
  } else {
    var luaSnippets = [];
    mem.validateLua.forEach(function(snip){
      luaSnippets.push(snip + "==false");
    });
    luaChecks = " if " + luaSnippets.join(" or ") + " then return false,'oops! '..'" + mem.memberName + "' end";
  }
...
function genLuaScripts(pageData: PageDef, outDir: string) {
  pageData.pageObjects.forEach(function(pObj) {
    ...
    const readGroups = pObj.readGroups ?? pageData.readGroups ?? ['root', 'admin'];
    const writeGroups = pObj.writeGroups ?? pageData.writeGroups ?? ['root', 'admin'];
    luaScript += `
      readGroups={${readGroups.map((group) => group + '=true').join(',')}},
      writeGroups={${writeGroups.map((group) => group + '=true').join(',')}},
    `;
    ...
    var luaGetScript_str = ["  get=function(authenticated, requestHandler)", "local o"].concat(luaGetScript, ["return o"]).join("\n    ") + "\n  end";
    var luaSetScript_str = ["  set=function(authenticated, o, requestHandler)"].concat(luaSetScript).join("\n    ") + "\n  end";
    var luaValidateScript_str = ["  validate=function(o, requestHandler)", "local v"].concat( luaValidateScript,["return true"]).join("\n    ") +  "\n  end";
