...
var ipPassthroughEnabled = false;
...
dhcpMustEnableNotice.setIsVisible(function() { return ipPassthroughEnabled;});

var DhcpCfg = PageObj("DhcpCfg", "DHCP configuration",
{
  members: [
    // TODO:: Need to support IP handover with multiple PDNs
    // hiddenVariable("ipPassthroughEnable", "service.ip_handover.enable"),
  ...
  decodeRdb: function(obj) {
    ...
    ipPassthroughEnabled = obj.ipPassthroughEnable == "1";
    if (ipPassthroughEnabled) {
...
var customLuaGetClientList = {
  ...
  get: function(arr) {
      arr.push("o = {}");
      arr.push("f = io.open('/tmp/dnsmasq-dhcp-lan.leases')")
      arr.push("if (f) then");
...
var customLuaStaticDhcp = {
  ...
  set: function() {
    return [
      ...
      "luardb.set('service.dhcp.trigger','1')",
      "return rc"
    ];
  },
  helpers: [
